...
| `bind-reachability-advisory.spec.ts` | L3 for the bind-vs-trust advisory (test-plan F1–F19, X5, X6): visibility vs bind host, draft-edit convergence with no save, inline `0.0.0.0` remediation + Server-page attribution of the dirty dot, coexistence with the block-event banner, restart-pending signal, WS push + connect replay, tailnet dropdown offers, unofferable `/32` row, endpoint-failure degradation. `reachability` is MERGED into the live `GET /api/config` (never a hand-built stub) and the guard surface normalised, else the harness's own seeded trust entries score the predicate. See change: warn-unreachable-trusted-networks. |
