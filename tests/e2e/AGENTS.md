...
| `ended-session-endedat.spec.ts` | L3 for the evidence-based `endedAt` invariant (test-plan #F1, #F2); plants historical transcripts in the… → see `ended-session-endedat.spec.ts.AGENTS.md` |
...
| `mcp-token-settings.spec.ts` | L3 for the Settings → Paired Devices MCP-client token flow (change: mcp-legacy-clients-and-token-issuance,… → see `mcp-token-settings.spec.ts.AGENTS.md` |
