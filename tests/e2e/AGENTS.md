...
| `tool-created-files.spec.ts` | L3 spec: tool-created files, failed Codex partial patch, failed Grok Shell. Asserts retained files, failure text, affected badge, diff open. See changes: detect-tool-created-files, retain-failed-tool-file-changes. |
