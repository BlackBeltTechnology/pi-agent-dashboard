...
| `ended-session-endedat.spec.ts` | L3 for the evidence-based `endedAt` invariant (test-plan #F1, #F2). Plants historical transcripts inside the harness via `docker exec` (container resolved from `.pi-test-harness.json`'s compose project) — a name WITHOUT the `<ts>_<uuid>` shape is scanner-invisible/bootstrap-visible, one WITH it is scanner-rebuilt. F1 asserts a bootstrap-restored record anchors at transcript mtime, not `startedAt`. F2 arms `completedFirst` and asserts the boot restore loop leaves stored order untouched; D1a's no-`onChange` guarantee is proven at L1, not here. See change: fix-ended-session-missing-endedat. |
