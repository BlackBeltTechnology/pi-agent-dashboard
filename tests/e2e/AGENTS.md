...
| `mcp-session-token.spec.ts` | L3 (change: wire-mcp-session-token): the REAL MCP session-token delivery path end to end. → see `mcp-session-token.spec.ts.AGENTS.md` |
