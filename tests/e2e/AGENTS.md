...
| `tmux-session-shutdown.spec.ts` | L3 gate on the scenario `fix-tmux-session-shutdown-leak` exists for (test-plan #T2): spawn under the harness's default `PI_SPAWN_STRATEGY=tmux`, `shutdown`, then assert the container's OWN process table — `/proc` + `tmux list-panes` via `docker exec`, out-of-band like `probe-harness-memory.mjs` — because a session RECORD disappearing is exactly what the bug already did. Reads the session pid from `/api/sessions` (the bus snapshot carries none). Container resolved by published `DASHBOARD_PORT`, never by name. This spec was RED while every L1 test was green. See change: fix-tmux-session-shutdown-leak. |
