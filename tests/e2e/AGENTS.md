...
| `browser-relay.spec.ts` | L3 for the browser relay (change: add-browser-relay, test-plan F1–F5 + F10). Needs the harness with `PI_E2E_SEED=1 PI_BROWSER_RELAY_FAKE=1`. → see `browser-relay.spec.ts.AGENTS.md` |
...
| `explicit-model-preserved.spec.ts` | L3 for `fix-default-model-clobbers-explicit-model` (test-plan #I1, #I2): launches a detached child pi process… → see `explicit-model-preserved.spec.ts.AGENTS.md` |
...
| `host-pressure-badge.spec.ts` | L3 for `fix-false-unresponsive-badge` (test-plan #F3, #F4). Drives a SYNTHETIC bridge straight at the pi gateway (`pairDeviceBearer` + `gatewayUrlWithTicket`, port from `/api/health#piGatewayPort`) and then says nothing — a real harness session heartbeats every 15 s and can never go quiet on demand. Borrows a real spawned session's `cwd` so the synthetic card renders in a visible folder group. F3: card silent while fresh → `data-host-pressure="unresponsive"` after 60 s of silence → pill gone after one `session_heartbeat`, all with NO page reload. F4: a SECOND browser context opened during the pressure learns the same state from `sessions_snapshot` alone. Exemplar: `bridge-contention-health.spec.ts`. |
...
| `mcp-client-settings-a11y.spec.ts` | L3 accessibility floor for the mcp-client settings section (change: extract-mcp-client-plugin, task 7.7). → see `mcp-client-settings-a11y.spec.ts.AGENTS.md` |
| `mcp-token-settings.spec.ts` | L3 for the Settings → Paired Devices MCP-client token flow (change: mcp-legacy-clients-and-token-issuance, F1/F2/F3): mint → one-shot token + `claude mcp add` snippet, dismiss-unrecoverable + `manual` badge, revoke → bearer dies on `/mcp`. Arms a session cookie (narrowed harness config, restored in afterAll) because the operator guard admits the docker-sourced browser only via `authVia: "session"`. → see `mcp-token-settings.spec.ts.AGENTS.md` |
...
| `quota-context-strip.spec.ts` | L3 (change: move-quota-to-context-strip; F7–F9, X3). Stubs `/api/quota`; asserts the `quota-context-group`… → see `quota-context-strip.spec.ts.AGENTS.md` |
