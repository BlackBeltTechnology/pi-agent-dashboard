...
| `browser-relay.spec.ts` | L3 for the browser relay (change: add-browser-relay, test-plan F1–F5 + F10). Needs the harness with `PI_E2E_SEED=1 PI_BROWSER_RELAY_FAKE=1` (docker has no Chrome → the plugin boots enabled and seeds one socket-less `Fake` instance with tab 1). F1–F4 drive the settings surface (cannot-open-Chrome, write-only token, kill switch clears + re-seeds the Fake, a denied audit row via `auditSeq`); F5/F10 mount the live-view tile on session select, subscribe over the core `/ws` (never a relay socket), render `browser_relay_frame` JPEGs, and unsubscribe on Close. |
...
| `model-favorites-cross-surface.spec.ts` | L3 for model-picker-everywhere-favorites (test-plan F3): star model in Settings → Sessions Default Model picker, session composer row shows `model-fav-toggle[aria-pressed=true]`, reload still pressed (server-persisted). Stubs `GET /api/models`; needs `PI_E2E_SEED=1`. See change: model-picker-everywhere-favorites. |
