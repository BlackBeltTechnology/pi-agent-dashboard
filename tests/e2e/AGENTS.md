...
| `openspec-board-worktree-availability.spec.ts` | L3 spec (test-plan #F6, change: fix-openspec-board-worktree-button-gating). Git-inits `/fixtures/openspec-board` out of band, spawns a session in the board cwd, ends it via `POST /api/session/:id/shutdown`, asserts every `card-new-worktree-*` stays PRESENT + ENABLED after `session_removed` — the old `gitBranch` gate made them vanish. |
