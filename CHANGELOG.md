...
- **A prompt no longer hangs at "sending" with the composer locked.** The bridge's inbound message pump could wedge on a model-catalogue refresh that never settled, stranding every later prompt; the pump is now serialized with a bounded queue and a timeout, control messages bypass the queue, and an unacknowledged prompt after 30s is marked failed with its text preserved instead of silently dropped. This also closes the `set_model` → `send_prompt` race (#412, #467).
...
- **Caught up with pi 0.71/0.72/0.73 UX features.** Four additions: (1) the Provider Authentication settings card detects catalogue OAuth rows with no matching server handler (e.g. an extension-registered provider) and renders their login button disabled with an explanatory tooltip instead of producing 400 errors, via a new `GET /api/provider-auth/handlers` endpoint; (2) per-model thinking levels — `ModelInfo` now carries `supportedThinkingLevels` derived from pi 0.72+'s per-model `thinkingLevelMap`, and `ThinkingLevelSelector` renders only the levels the active model supports (falls back to all six pre-0.72); (3) graceful stop-after-turn — a new "Stop after turn" button (next to Abort / Force Kill) lets the agent finish the current turn then shut the session down cleanly, via a new `stop_after_turn` browser→server→bridge message; (4) streaming bash output — large tool results now keep the LAST 200 lines with a `«N earlier lines hidden»` marker (was first 30), plus an on-demand "Show full output" affordance backed by `GET /api/sessions/:sessionId/tool-result/:toolCallId`. (change: `adopt-pi-071-072-073-features`)
- **pi-version skew advisory restored + per-session pi version.** Two complementary surfaces re-attach the dormant `pi-version-skew.ts` primitives. (1) Global advisory: `GET /api/health` gains a `compatibility` field (30s-cached `readCurrentPiVersion` + `computeCompatibility`, `null` when pi unresolvable), and Settings → General renders a non-blocking advisory — amber soft pill below `recommended`, red panel with a copy-paste `npm install -g @earendil-works/pi-coding-agent@<recommended>` disclosure below `minimum` (`computeCompatibility` now emits a populated `error` string naming both versions). (2) Per-session version: the bridge reports the pi version its session actually runs (read via `createRequire` from inside pi's own tree) through a new additive `pi_version_update` message pushed at register and on change via the existing 30s git/model poll tick; the server stores it as `DashboardSession.piVersion` and re-broadcasts, and the session header shows a `pi <version>` label. Out-of-band upgrades (`pi update --self`, `npm i -g`) reflect within ~30s without a restart. Folds in the one surviving idea from the retired `modernize-pi-version-handling`. (change: `restore-pi-version-skew-surface`)
...
- **Assistant message text now honors pi 0.71+ extension `message_end` content replacement** (e.g. footers, redactions). Previously the live render kept delta-derived text while `/reload` showed the replacement, producing a visible drift; the reducer's `message_end` arm now derives the effective text uniformly (new `deriveEffectiveAssistantText` helper) across the already-flushed, streaming-row, and replay branches. (change: `adopt-pi-071-072-073-features`)
- **Node-version predicates unified in one place; the Electron doctor now agrees with the server about which system Node is usable.** The nodejs/node#58515 affected range and the `engines.node` cap were encoded in three drifting copies; the Electron `dependency-detector.ts` copy still used the old `v22.x < 18` cutoff, so the doctor reported a system Node v22.18 (and v21.x, and v26+) as usable even though the server refuses to start on them. The two predicates now live once in `packages/shared/src/node-version.ts` (`isAffectedNode`, `isOutOfEnginesRange`, plus the combined `isUsableNodeVersion`); `node-guard.ts` re-exports them and the doctor's `detectSystemNode` / on-disk scan gate on the shared `isUsableNodeVersion`. Net: the doctor reports a system Node usable iff the server accepts it — Node 22.19+, 24, and 25 are usable; 22.0–22.18, 24.1–24.2, 21.x, and 26+ are not. (change: `unify-node-version-gate`)
