...
### Changed
- **Pi compatibility floor lifted to 0.76.0 (recommended 0.76.0).** Tracks the latest upstream pi-coding-agent release; no Node engines change (0.76 inherits 0.75's `>=22.19.0` Node floor). Bundled-extension peer-deps bumped to `>=0.76.0` / `^0.76.0` in lockstep. (change: `bump-pi-compat-to-0-76`)
