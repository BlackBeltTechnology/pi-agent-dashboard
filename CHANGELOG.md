...
### Added
- **Standalone npm install no longer requires pre-installing pi.** `npm install -g @blackbelt-technology/pi-agent-dashboard` is now sufficient; the dashboard CLI bootstraps pi + openspec into `~/.pi-dashboard/` on first run. `jiti` is now a direct dependency of `@blackbelt-technology/pi-dashboard-server` so the bin wrapper resolves it from its own `node_modules` via the `argv[1]` walk-up — no system pi install needed to bring the server up. On first launch, `cli.ts::maybeSeedDefaultInstallableList()` writes a default `~/.pi/dashboard/installable.json` (pi + openspec, both `required: true`, `kind: "npm"`) when neither the file nor a managed pi is present and `DASHBOARD_STARTER !== "Electron"`. The server binds immediately; `bootstrapInstallFromList` installs in the background and session-spawn endpoints return 503 with a bootstrap-in-progress message until ready (UI banner from `useBootstrapStatus`). Electron's wizard-driven seeding is untouched. (change: `enable-standalone-npm-install`)
