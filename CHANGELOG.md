...
- **Design sandbox infrastructure for visual UI mockups.** Docker-based sandbox (`sandbox/Dockerfile`, `sandbox/docker-compose.yml`, `sandbox/entrypoint.sh`) runs `pi-dashboard` with version-controlled seed data (`seed/`) covering five workspace types (active project, empty, openspec-heavy, multi-folder, error states). Headless Chromium (`chromedp/headless-shell`) captures before-screenshots via scenario-driven browser automation (`browser-visual-debug --sandbox --scenario`). Vision-capable `sandbox-designer` skill produces Tailwind HTML mockups from screenshots + user stories. `openspec-propose` gains an optional Docker-gated Design Phase; `openspec-archive-change` applies `seed.patch` / `Dockerfile.patch` before archive, growing seed data organically. No dashboard source code changes — infrastructure and skills only, developer tooling.
