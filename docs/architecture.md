...
- **`src/slot-consumers.tsx`** — one component per slot id. Each wraps contributions in a `SlotErrorBoundary` (per-claim scope). Reads registry from the provider. `ShellOverlayRouteSlot` selects its container from the claim's effective `presentation` (default `"dialog"`; `"page"` opts out → full viewport desktop + mobile). Dialog container injected by host via `dialogContainer` prop, NOT imported — avoids `client-utils` → `dashboard-plugin-runtime` dependency cycle. See change: add-route-backed-overlay-dialogs.
