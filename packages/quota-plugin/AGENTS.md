...
| `package.json` | Manifest `id: quota`, `displayName: Provider Quota`, priority 600, `client`+`server` entries (NO bridge), `configSchema`, claims `content-inline-footer`→`QuotaWidget` + `settings-section`→`QuotaSettings` (tab general). `private: true` (bundled). Deps: dashboard-plugin-runtime, `pi-dashboard-server` (workspace:* — host auth abstraction), shared, `@latentminds/pi-quotas` pinned `0.4.0` (deep raw-TS import; no `exports` map upstream). |
