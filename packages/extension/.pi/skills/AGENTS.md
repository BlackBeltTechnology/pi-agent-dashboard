...
| `pi-dashboard/commands/dashboard-list-models.md` | LLM-bound command. GET $BASE/api/models; `annotated` arg → /api/models?annotated=1 adds `excludedReason`… → see `pi-dashboard/commands/dashboard-list-models.md.AGENTS.md` |
...
| `pi-dashboard/SKILL.md` | Bundled skill: monitor + control dashboard from any pi session. Updated for archive/unarchive verbs and `/api/sessions/archived` endpoints. See change: archive-sessions-lazy-load. → see `pi-dashboard/SKILL.md.AGENTS.md` |
...
| `project-init/SKILL.md` | Bundled skill: scaffold bare dir into configured pi project. → see `project-init/SKILL.md.AGENTS.md` |
