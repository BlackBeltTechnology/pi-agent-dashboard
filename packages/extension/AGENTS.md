...
| `vitest.config.ts` | Vitest config for extension package. Includes `src/**/__tests__/**/*.test.ts`, node env, `forks` pool, `maxWorkers: "50%"`, globalSetup `setup-home.ts`, per-file HOME isolation via `setup-home-perfile.ts` for `providers.json` write races. `testTimeout` 30_000 — session/agent command tests blew the 5s default under fork contention. See change: contention-harden-real-process-tests. |
