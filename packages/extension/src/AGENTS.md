...
| `autostart-guard.ts` | Worktree auto-start refusal + durable log. Exports `isWorktreeCliPath` (matches a `.worktrees` segment pre- AND post-realpath), `shouldRefuseWorktreeAutoStart` (worktree AND default 8000/9999 port), `appendAutoStartLog` (creates the log file the skipped launch primitive would have). See change: fix-worktree-server-autostart-leak. |
| `autostart-lock.ts` | Single-flight auto-start lock at `~/.pi/dashboard/autostart-<port>.lock` (atomic `open 'wx'`). Exports `acquireAutoStartLock`, `releaseAutoStartLock`, `recordChildPid`, pure `isLockStale` (dead holder AND dead child, pid-reuse via `startedAt`, or age past the readiness budget). Unwritable dir degrades, never throws. See change: fix-worktree-server-autostart-leak. |
