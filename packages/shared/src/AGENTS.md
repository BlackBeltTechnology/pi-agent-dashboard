...
| `bind-reachability.ts` | Single home of the bind-vs-trust predicate + well-known-range table; BOTH packages import it, so client/server cannot drift. Exports `unreachableTrustedEntries` (loopback-only entry → non-IPv4 fail-open → `0.0.0.0` → malformed skip → loopback bind → containment), `collectTrustedEntries` (union of `trustedNetworks` + `auth.bypassHosts`), `resolveBindHost`/`pendingEffectiveHost` (`--host` → `PI_DASHBOARD_HOST` → `config.bindHost` → `127.0.0.1`; unsaved draft outranks), `wellKnownContainingRange`, `interfaceLabel`, `deriveInterfaceSuggestions`, `dedupeInterfaceOffers`, type `BindReachability`. ADVISORY only — never gates a request; an address test, not a routing test. See change: warn-unreachable-trusted-networks. |
