...
| `config.ts` | Dashboard config loader. `loadConfig()` reads `~/.pi/dashboard/config.json` via `CONFIG_FILE`;… → see `config.ts.AGENTS.md` Also the single source of truth for startup timing (`HEALTH_CHECK_TIMEOUT_MS`, `SPAWN_READINESS_BUDGET_MS` = 3×, `SERVER_STARTUP_DEADLINE_MS` derived) and the shared production ports (`DEFAULT_DASHBOARD_PORT`, `DEFAULT_GATEWAY_PORT`) the worktree refusal keys on — kept HERE, not in a new module, because a brand-new shared file is unresolvable from a git worktree and drops the server into recovery mode. See change: fix-worktree-server-autostart-leak. |
