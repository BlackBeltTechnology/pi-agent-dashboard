...
- Stale-path reclamation: bind writes `<path>.pid` (`0600`); an `indeterminate` probe may unlink only when that pid is recorded AND `isProcessAlive` says gone. Missing/unparseable pidfile → refuse. A `live` probe always overrides the pidfile. Without this a SIGKILLed dashboard wedges its own path forever (@review finding 1, D9 cycle-5 amendment).
