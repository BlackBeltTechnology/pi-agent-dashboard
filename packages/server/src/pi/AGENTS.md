...
| `bridge-upgrade-auth.ts` | Pure per-transport gate for bridge WS upgrades. `decideBridgeUpgrade({transport,remoteAddress?,url?,secWebSocketProtocol?,requireTicketOnLoopback?,consumeTicket})` → allow/refuse with a named cause; `isLoopbackAddress`. unix ⇒ allowed uncredentialed (kernel decided, D5); remote TCP ⇒ valid single-use bridge-scoped ticket, no grace; loopback TCP ⇒ bounded deprecation window (refused from 1.0.0). Installed as `verifyClient` on the TCP listener in `pi-gateway.ts::start`. See change: add-pi-gateway-transport-identity (D10b, task 6.3). |
