...
| `pi-gateway.ts` | WebSocket server for bridge extension connections. Routes `ExtensionToServerMessage` → `SessionManager`;… → see `pi-gateway.ts.AGENTS.md` `onSessionRegistered(sessionId, cwd, spawnToken?)` forwards `session_register.spawnToken` so cwd-keyed pending registries can claim the entry belonging to exactly that spawn. See change: fix-automation-stamp-correlation. |
