...
| `system-routes.ts` | REST routes: config, health, shutdown, tunnel. External-editor endpoints `/api/editors`,… → see `system-routes.ts.AGENTS.md` `/api/health` gains a `subagentTickThrottle` roll-up: reduces `activeSessions` `processMetrics` into summed `tickForwarded`/`tickCoalesced`/`tickDiscardedAtTerminal`/`tickDroppedNotReady`, beside the other silent-loss counters (also surfaced per-session on `agents[]`). See change: reduce-bridge-tick-bandwidth. |
