...
| `meta-persistence.ts` | Per-session debounced `.meta.json` writer. Exports `MetaPersistence`, `createMetaPersistence`. → see `meta-persistence.ts.AGENTS.md` Adds `flush(sessionFile)` and carries `archived`/`archivedAt`/`restoredAt` forward across routine writes. See change: archive-sessions-lazy-load. |
