...
| `session-diff-cache.ts` | `SessionDiffCache<T>` (TTL result cache + single-flight coalesce; TTL 0 disables; LRU-ish eviction past `maxEntries`) + `djb2(str)` non-crypto hash for the dirty-signature key component. Backs `/api/session-diff` repeated-poll coalescing. See change: fix-session-diff-eventloop-block. |
