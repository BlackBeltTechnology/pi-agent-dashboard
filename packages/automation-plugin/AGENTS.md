...
| `src/__tests__/single-flight-per-invoice.test.ts` | Per-invoice dispatch single-flight regression tests. Scheduler second-fire refusal; new-id still runs; scheduler↔manual runInvoice shared guard; release and redispatch after session death and normal completion. See change: guard-single-flight-invoice-dispatch. |
