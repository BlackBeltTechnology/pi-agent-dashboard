...
Operator setup guide: [`docs/chat-gateway.md`](../../docs/chat-gateway.md) — Discord bot creation, token, `allowedRoots`, pairing, L3 guard, team controls (tier model + chokepoint, per-binding mappings, workspace-scope narrowing, mirror filter boundary, disarm/rollback). See change: add-chat-gateway, add-chat-gateway-team-controls.

| File | Purpose |
|------|---------|
| `README.md` | Plugin overview: Discord-first platform adapter contract (vendored MIT `base.ts`), settings section (`ChatGatewaySettings`), inbound bind/attach/resume/spawn → `send_prompt`, outbound in-place streaming + native controls, L3 tool guard. States the inert-without-token posture and the real-path `allowedRoots` spawn boundary. Points at [`docs/chat-gateway.md`](../../docs/chat-gateway.md) for setup. |
