...
| File | Purpose |
|------|---------|
| `types.ts` | The pure shared contract (no I/O, no host import): `ChatGatewayConfig` (mirrors `../configSchema.json`) + `ResolvedConfig` + `CONFIG_DEFAULTS`, `InboundMessage`, `BindingSource` (precedence-ordered), `Binding` + `bindingKey({platform,channelId,threadId})` (canonical, collision-free; thread keyed separately from its parent channel), `AuthAction`/`AuthDecision`. Imported by both the server component and the settings panel. |
