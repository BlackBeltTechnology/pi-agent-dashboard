...
| `tsconfig.json` | Extends `../../tsconfig.base.json`. jsx react-jsx, noEmit. No vendor `paths` — the vendored relay's specifiers are package-relative, so nothing resolves through tsconfig. |
| `vitest.config.ts` | Vitest project. jsdom, forks pool, react plugin, `PARALLEL_MAX_WORKERS`. `resolve.alias` maps ONLY `@blackbelt-technology/pi-dashboard-shared`→worktree-local src (worktree-local shared wins over the hoisted symlink). No vendor aliases: the vendored relay imports `../../../../shims/*.js` relative to itself. Registered in ROOT `vitest.config.ts` `test.projects`. |
