...
| `vite.config.ts` | Vite config for client package. Resolves dashboard port via `PI_DASHBOARD_PORT` env → `/tmp/dash-dev-port` marker → `~/.pi/dashboard/config.json` → fallback `8000`. Aliases shared + client-utils sources. `viteDashboardPluginsPlugin` loads dashboard plugins from repo root. `manualChunks` splits react-vendor, markdown, syntax, diff, xterm, dnd, util, monaco chunks. Dev server proxies `/api` + `/ws` to dashboard port.  D1: npm `diff` split into its own `jsdiff` chunk so the chat path stops pinning `@git-diff-view/*`. See change: add-lazy-terminal-diff-bootstrap. |
