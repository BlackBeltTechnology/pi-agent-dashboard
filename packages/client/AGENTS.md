...
| `vite.config.ts` | Vite config for client package. Resolves dashboard port via `PI_DASHBOARD_PORT` env → `/tmp/dash-dev-port` marker → `~/.pi/dashboard/config.json` → fallback `8000`. Aliases shared + client-utils sources. `viteDashboardPluginsPlugin` loads dashboard plugins from repo root. `manualChunks` splits react-vendor, markdown, syntax, diff, xterm, dnd, util, monaco chunks. Dev server proxies `/api` + `/ws` to dashboard port. `manualChunks` isolates npm `diff` in its own `jsdiff` chunk — separate from `@git-diff-view/*` (`diff`) — so build/runtime tests distinguish the jsdiff-only path from the rich-diff chunk. See change: add-lazy-terminal-diff-bootstrap. |
