...
Older-page prepend anchoring: `handleScroll` snapshots `{sessionId,key,index,offset}` only when it sends `onLoadOlder`. Layout restore runs only when that same row reappears at a higher index, then `scrollToIndex` + rAF offset preserves reader position. Tail appends, flat row counts from tool-burst grouping, key disappearance, session switches, and bottom-pinned mode never force a jump. See change: fix-older-history-scroll-anchor.
