...
| `SessionTabBar.tsx` | Desktop browser-tab-style bar of pinned ("kedvenc") sessions above the content area. Renders one tab per `pinnedSessions` id resolvable in the live `sessions` map, server-persisted order; click → `navigate(/session/:id)`, ✕ → `unpin_session` (non-destructive). Status dot + `unread` mark + `endedAt` dims + "ended". `overflow-x-auto` on overflow. Exports `SessionTabBar`. See change: session-tab-bar. |
