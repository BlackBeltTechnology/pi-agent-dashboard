...
| `SessionList.tsx` | Main sidebar session list. DnD-ordered (`@dnd-kit`) pinned/unpinned + workspace tiers, folder grouping,…… → see `SessionList.tsx.AGENTS.md` Folder section render gate readiness-driven; `openspecOfferInitialization`/`openspecEnabled` props; conditional “Enable OpenSpec for this folder” DIRECTORY menu item; `seekToFolderOpenSpec` (expand+scroll+focus, no dialog). See change: add-openspec-init-affordances. `folderIsGitRepo` + the folder `+ New Worktree` gate both delegate to `resolveWorktreeAvailability` (fail-open, `folderGitMap`-aware), so a zero-session pinned git folder keeps the button. See change: fix-openspec-board-worktree-button-gating. Adds stub groups from `endedTotals` + ended expander/paging (`sessions_page`, per-group in-flight). See change: fix-connect-snapshot-frame-loss. |
