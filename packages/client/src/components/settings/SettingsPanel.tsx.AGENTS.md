...
- `unit` renders INSIDE the `<label>`, so it forms part of the accessible name. Units were stripped from the label text of 9 keys across en / zh-CN / hu — the translated strings embedded the unit, so English-only stripping would double it in those locales. Guarded by `__tests__/settings-unit-i18n.test.tsx`.
