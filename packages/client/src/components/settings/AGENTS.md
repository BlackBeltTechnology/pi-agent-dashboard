...
| `RetrySettingsSection.tsx` | Editor for pi's OWN retry policy (`retry.{enabled,maxRetries,baseDelayMs}`) via `pi-retry-api`. Exports `RetrySettingsSection`; `load`/`save` props injectable for tests. Mounted in SettingsPanel `providers` tab after "API Proxy". Renders enabled toggle (off → greys the numeric fields), the two numeric fields, a live `SchedulePreview` (delay progression + total, since pi's delay is `baseDelayMs * 2^(n-1)` uncapped), a NON-BLOCKING long-tail warning above `LONG_TAIL_ATTEMPTS=20` (warned, never capped — a UI cap is bypassed by editing the file), a global-scope disclosure, and Save reporting `reloadedSessions`. Client-side validation mirrors the server's. `retry.provider.*` deliberately NOT surfaced (emits no events → invisible wait). See change: retry-forever-with-stop-control. |
