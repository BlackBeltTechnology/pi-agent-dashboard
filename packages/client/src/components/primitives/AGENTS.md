...
| `CopyButton.tsx` | Clipboard copy button with copied-state check icon. Exports `CopyButton`. Calls `navigator.clipboard.writeText`; resets state after 1500ms. Fails silently when Clipboard API unavailable. Routes through `lib/util/clipboard.ts#copyText` (writeText → hidden textarea + `execCommand("copy")` fallback); ✓ only on a `true` result, still silent on total failure. See change: fix-long-session-ux-degradation (§2). Optional `testId` prop (See change: redesign-directory-card). Revert timer held in a ref: cleared on unmount + restarted on re-click, so no 1500ms callback survives the tree (an uncleared one fired post-jsdom-teardown → `ReferenceError: window is not defined` in unrelated suites). |
