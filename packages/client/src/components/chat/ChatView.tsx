...
/**
 * Inline terminal card, deferred. A transcript with terminal history DOES fetch
 * the terminal chunk during replay (design D3b — a deliberate scoped carve-out);
 * a transcript without one never reaches `@xterm/*`. Card-LOCAL boundary so the
 * transcript stays scrollable while the chunk loads.
 *
 * See change: add-lazy-terminal-diff-bootstrap (D2/D3b).
 */
const InlineTerminalCard = lazy(() => import("../terminal/InlineTerminalCard.js").then((m) => ({ default: m.InlineTerminalCard })));

/** In-surface loading affordance for the inline terminal card. */
function InlineTerminalCardFallback({ failed = false }: { failed?: boolean }) {
  return (
    <div className="rounded border border-[var(--border-secondary)] p-3 text-xs text-[var(--text-tertiary)]">
      {failed ? "Terminal failed to load." : "Loading terminal…"}
    </div>
  );
}
    ...
    <div ref={scrollRef} onScroll={handleScroll} onCopy={handleCopy} onWheel={cancelDescent} onTouchMove={cancelDescent} style={{ overflowAnchor: "none" }} data-testid="chat-scroll-container" className={`chat-cv h-full overflow-y-auto ${isMobile ? "p-2" : "p-4"}`}>
      ...
      <div className="chat-cv-skip" style={{ position: "relative", width: "100%", height: totalSize }}>
        ...
        if (msg.role === "inlineTerminal") {
          const args = msg.args as any;
          return (
            <ErrorBoundary fallback={<InlineTerminalCardFallback failed />}>
              <Suspense fallback={<InlineTerminalCardFallback />}>
                <InlineTerminalCard
                  key={msg.id}
