...
| `GrammarPanel.tsx` | Corrections panel above the composer (sibling to QueuePanel in App). Renders diff-highlighted corrections (struck `original` in `--accent-red` → `replacement` in `--accent-green`), a grammar summary, per-suggestion Accept (disabled when `stale`)/Dismiss, an Apply-all button, and a close control. States: idle→null, checking, error (maps `GrammarErrorCode`→i18n message), done-no-issues, done-with-suggestions. Driven by `useGrammarCheck`. i18n keys `grammar.*`; the composer Check button + ⌘G shortcut live in `CommandInput.tsx`. See change: add-composer-grammar-check. |
