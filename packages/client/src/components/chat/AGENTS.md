...
| `CommandInput.tsx` | Chat composer textarea + autocomplete. Exports `CommandInput`, `parseViewCommand`, `shouldWalkFileQuery`,… → see `CommandInput.tsx.AGENTS.md` `pendingPrompt` prop is now the status union, not a boolean; only `"sending"` gates the composer. See change: fix-optimistic-prompt-stuck-sending. |
