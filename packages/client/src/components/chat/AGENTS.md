...
| `CommandInput.tsx` | Chat composer textarea + autocomplete. Exports `CommandInput`, `parseViewCommand`, `shouldWalkFileQuery`,… → see `CommandInput.tsx.AGENTS.md` New `modelRefreshErrors?` prop forwarded to `ModelSelector.refreshErrors`. See change: upgrade-model-selector-primitives. |
