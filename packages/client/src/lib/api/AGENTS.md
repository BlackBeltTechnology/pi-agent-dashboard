...
| `models-api.ts` | `fetchModelCatalogue()` → discriminated ok/unavailable result over `GET /api/models`, so an empty catalogue never reads as a failure. 10s client bound, raced not merely aborted. |
...
| `pi-retry-api.ts` | Fetch helpers for pi's retry policy against `GET/PUT /api/pi-retry`. Exports `getPiRetryPolicy()`, `putPiRetryPolicy(policy)` → `{policy,reloadedSessions}`, `PiRetryPolicy` re-export, `PI_RETRY_DEFAULTS`. Throws on `success:false`. See change: retry-forever-with-stop-control. |
| `pi-runtime-api.ts` | Fetch helpers for pi runtime discovery + selection: `fetchPiInstalls()` (`GET /api/pi/installs`) and `setPiRuntime(selection)` (`POST /api/pi/runtime`). ONE request carries BOTH consumers — two sequential writes can fail between them and leave the runtime split in half. Re-exports `PiInstallEntry`, `PiInstallsResponse`, `SetPiRuntimeRequest` from shared `rest-api.ts`. See change: select-pi-runtime-install (design D7). |
