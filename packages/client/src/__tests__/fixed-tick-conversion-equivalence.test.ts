...
const CENSUS: Record<string, string[]> = {
  ...
  "components/__tests__/PathPicker.test.tsx": [
    ...
    // Raw source bytes: the extractor returns text between the quotes verbatim
    // (no escape processing), so backslash sequences below are literal.
    String.raw`shows inline 'Create \"<name>\" here' row when partial has no exact match`,
    ...
    String.raw`Enter on C:\\Users\\me\\ calls onSelect with the input value and closes`,
    String.raw`Select button on C:\\Users\\me\\ calls onSelect`,
    String.raw`Enter on UNC \\\\server\\share\\ calls onSelect`,
