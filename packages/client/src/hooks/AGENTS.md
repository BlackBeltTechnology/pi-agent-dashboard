...
| `useCustomEntryPayload.ts` | On-demand fetch hook `useCustomEntryPayload(sessionId, entryId) → { payload?, error?, loading, fetchPayload }` for a custom entry's FULL payload from `GET /api/sessions/:sid/entry/:id`. Mirrors `useToolFullResult`: skips when either id is missing, 404 → `entry evicted`, never throws (a failed fetch degrades the renderer to the stored body). Collapsed-first: `fetchPayload` is called ONLY from the expand path, so at-rest rows issue zero requests. See change: add-custom-entry-renderer-slot. |
...
| `useMessageHandler.ts` | New `case "view_messages_update"`: replaces `viewMessagesMap.get(sessionId)` with `msg.viewMessages.slice()`. → see `useMessageHandler.ts.AGENTS.md` Adds `sessions_page_result` merge, `openspec_get_result` apply, snapshot `endedTotals`→`endedTotalsMap` + `snapshotGeneration`, live endedTotals, tail-keeping `sessions_reordered`. See change: fix-connect-snapshot-frame-loss. Handles `session_archived` (deletes session, updates count + ended totals), `archived_count_updated`, and snapshot `archivedCountByCwd`. See change: archive-sessions-lazy-load. Reconciled `session_added` takes the server record as authoritative (a plain merge would keep a stale `currentTool`/`hostPressure` from the previous incarnation) and carries over only the client-local `resuming`/`closing`/`assets`; gates navigation off ALL tiers (placeholder/pending-spawn cleanup only on an exact `spawnRequestId`); appends a newly added id to the group order tail; an add flipping a held ended row back to live removes its `endedTotals` contribution; paging `pageReplyGen` (release in-flight on any reply) + `pageExhausted` (hide "more" until `endedTotals` changes). See change: close-registry-frame-shed-gaps. |
