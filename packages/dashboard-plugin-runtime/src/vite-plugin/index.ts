 ...
 */
/**
 * Reject a cross-plugin `(custom-entry-renderer, customType)` collision.
 *
 * `manifest-validator` only checks duplicates WITHIN one plugin; the browser
 * registry that resolves react claims is assembled here, so this is the one
 * place an aggregate check can run before the bundle is emitted. Two plugins
 * owning one `customType` is fatal by design (no priority/id tiebreak): a
 * silent winner would leave the losing plugin's author with no signal at all.
 *
 * Throws naming BOTH plugin ids and the conflicting `customType`; the caller
 * therefore emits no registry file.
 * See change: add-custom-entry-renderer-slot (design D2).
 */
function assertNoCrossPluginCustomTypeCollisions(entries: PluginEntry[]): void {
  const owners = new Map<string, string>();
  for (const entry of entries) {
    for (const claim of entry.manifest.claims) {
      if (claim.slot !== "custom-entry-renderer" || !claim.customType) continue;
      const existing = owners.get(claim.customType);
      if (existing && existing !== entry.manifest.id) {
        throw new Error(
          `[vite-dashboard-plugins] Fatal custom-entry-renderer collision: plugins ` +
            `"${existing}" and "${entry.manifest.id}" both claim customType ` +
            `"${claim.customType}". Each customType must be claimed by exactly one plugin.`,
        );
      }
      owners.set(claim.customType, entry.manifest.id);
    }
  }
}

function generateRegistryContent(entries: PluginEntry[], repoRoot: string): string {
  assertNoCrossPluginCustomTypeCollisions(entries);
  ...
  for (const entry of entries) {
    ...
    for (const claim of manifest.claims) {
      ...
      // Keyed `customType` for custom-entry-renderer claims. Required for the
      // client's `forCustomType` resolution to see the key at all.
      // See change: add-custom-entry-renderer-slot.
      const customTypeStr = claim.customType
        ? `, customType: ${JSON.stringify(claim.customType)}`
        : "";
      ...
      lines.push(
        `      { pluginId: ${JSON.stringify(manifest.id)}, priority: ${manifest.priority ?? 1000}, slot: ${JSON.stringify(claim.slot)}${tabStr}${toolNameStr}${customTypeStr}${commandStr}${pathStr}${sessionParamStr}${depthStr}${parentPathStr}${presentationStr}${configStr}${componentRef}${predicateRef}${shouldRenderRef} },`,
      );
    }
    lines.push("    ],");
