...
| `font.ts` | `glyphText(merged)` (every rendered string) + `subsetFont(font, text)` (opentype.js rebuild with only the used glyphs) + `freezeHeadDates` (zeroes `head.created`/`modified` so subset bytes are clock-independent). |
