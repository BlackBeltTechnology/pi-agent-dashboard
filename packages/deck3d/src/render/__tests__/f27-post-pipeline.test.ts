...
const frame = async (page: Page, t: number): Promise<string> => {
...
/**
 * Pixel diff of the page's canvas against another frame. Multi-pass half-float
 * blurs (bloom) are not bit-exact across GPU runs — a handful of pixels drift
 * by 1–2 levels — so "deterministic" for a frame means within this tolerance.
 */
const frameDiff = (page: Page, other: string): Promise<{ differing: number; total: number; maxDelta: number }> =>
...
const expectSameFrame = async (page: Page, other: string, label: string): Promise<void> => {
...
/** Mean luminance of the rendered canvas inside a CSS-px rect (reads the WebGL buffer directly). */
const meanLum = (page: Page, rect: { x: number; y: number; w: number; h: number }): Promise<number> =>
  page.evaluate((r) => {
