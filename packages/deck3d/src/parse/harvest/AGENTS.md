...
| `bundle.ts` | `buildHarnessBundle` (esbuild IIFE → `dist/harvest/harness.js`), `getHarnessBundle` (rebuild when missing or older than `harness.ts`), `renderHarnessHtml` (`__BUNDLE__`/`__FONT__`/`__STALL__` substitution, `</script` escaped). Paths via `pkgRoot()` so a bundled `dist/cli.js` resolves the same `src/` + `dist/` locations. |
