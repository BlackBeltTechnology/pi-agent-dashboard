...
| `derive.ts` | `parseDeck`/`deriveDeckIR`: parsed markdown → `DeckIR`. Regenerates `defaults`+`slides`, preserves prior `overrides`, applies inline overrides (warn on clobber), emits orphan-override warnings, injects the mermaid `Harvester`, computes `meta.derivedHash`. |
