...
export async function registerPlugin(ctx: ServerPluginContext): Promise<void> {
  ...
  // NOTE (D18): this plugin is a token RESOLVER only — it validates incoming
  // bearer tokens and does NOTHING else by itself. It does not publish a
  // browser login descriptor and ships no login/logout UI. Advertising login
  // (via `ctx.registerBrowserLoginConfig`) and claiming the core
  // `login-provider` slot belong to a separate authorization plugin that owns
  // the sign-in/out UX and imports the shared mechanics from client-utils.
}
