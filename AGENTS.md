...
| a kb hit shows `STALE` / `GONE` / `MOVED` (trust verdict) | verify the row against source before acting — `FRESH` may be acted on without re-reading; see `kb_search` verdicts (trust label, never ranking) |
