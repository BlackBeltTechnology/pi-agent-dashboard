...
- [x] 1.1 In `packages/shared/src/state-replay.ts` `replayEntriesAsEvents`, special-case
...
- [x] 1.2 After the entry loop (before the flow-event emission), if `latestGreeting` is
...
- [x] 1.3 Non-greeting `custom_message` entries keep their current inline
      `message_start` + `message_end` emission unchanged (per-entry `entryId`).
- [x] 1.4 `display` falsy / absent greeting → still skipped (the existing
      `entry.display` guard), no singleton handling.

## 2. Reducer — greeting replaces in place (live + replay path)

- [x] 2.1 In `packages/client/src/lib/chat/event-reducer.ts` `message_end`, in the
...
- [x] 2.2 Non-greeting custom messages keep `custom-<entryId>` (unchanged, not
      collapsed). `display` falsy → still no row.

## 3. Lock test — query route passthrough

- [x] 3.1 In `packages/invoicebot-plugin/src/server/__tests__/routes.test.ts`, add a lock
...
- [x] T1 Replay singleton: three `ib-greeting` entries (A, B, C) replay to exactly one
      `message_start` + one `message_end`, both content C, `entryId` = latest id, at the
      first greeting's slot.
- [x] T2 Replay isolation: `ib-greeting` entries + an unrelated `x-note` entry replay to
      one greeting pair + one `x-note` pair (x-note content + entryId intact).
- [x] T3 Replay no-greeting: display-flagged custom messages with no `ib-greeting` replay
      unchanged (regression guard).
- [x] T4 Reducer replacement: two `ib-greeting` `message_end`s (A then B) → exactly one
      row, id `custom-ib-greeting`, content B.
- [x] T5 Reducer isolation: `ib-greeting` (A then B) + one `x-note` → one greeting row
      (B) + one separate `x-note` row.
- [x] T6 Reducer hidden: `customType:"ib-greeting"`, `display:false` → no row.

## Validate

- [x] V1 `openspec validate replace-replayed-greeting --strict` passes.
- [x] V2 Scoped tests green: `packages/shared` replay suite, `packages/client`
