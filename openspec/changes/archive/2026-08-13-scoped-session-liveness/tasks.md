...
- [x] 1.1 In `src/server/session-link.ts`, confirm `recordedUsableSession` /
...
- [x] 1.2 Ensure the canonical-adoption gate ADOPTS that bound scoped session and
  returns its live/auto-resumable id, with **no dashboard-initiated spawn** when
  it exists.
- [x] 1.3 Confirm the global-never-adopted guard still rejects shared
  `invoicebot-intake` / `invoicebot-pull` / `ask` sessions as canonical.
- [x] 1.4 Confirm `POST /api/plugins/invoicebot/scoped-session` spawn remains a
...
- [x] 2.1 In `src/bridge/index.ts` / `src/server/index.ts`, confirm
  `payload.data.invoice_id` is preserved verbatim on `ib_invoice_state_changed`
  (as it is on `ib_invoice_cost_updated`).
- [x] 2.2 Confirm `ib_invoice_state_changed` is forwarded for every observed
  transition (no drop/coalesce), on the same `ib_domain_event` road as cost.

## Tests (faux, zero-network)

- [x] T.1 session-link: a bound `invoicebot-scoped:<invoice_id>` recorded run is
  adopted as canonical and returned WITHOUT a spawn (assert spawn not called).
- [x] T.2 session-link: an invoice with only a shared `invoicebot-intake`
  recorded run yields NO canonical adoption of that intake session.
- [x] T.3 session-link: an invoice with no bound scoped session triggers NO
  proactive spawn outside the explicit `POST /scoped-session` call.
- [x] T.4 event-bridge: `ib:invoice-state-changed` forwards
  `ib_invoice_state_changed` with `payload.data.invoice_id` preserved verbatim.
- [x] T.5 event-bridge: a sequence of mid-flight `ib:invoice-state-changed`
  emissions each forward one frame (none dropped/collapsed).

## Validate

- [x] V.1 `cd packages/invoicebot-plugin && npm test` (or the repo faux suite)
  green for the new/adjusted specs.
- [x] V.2 `openspec validate scoped-session-liveness --strict` passes.
- [x] V.3 Manual (deferred, tested at gate): open an invoice before processing,
