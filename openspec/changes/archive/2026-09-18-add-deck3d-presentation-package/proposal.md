...
- **Effects as a corpus, not code per deck.** Every effect in the lab (bloom, mirror floor, glass, token/ring/swarm backgrounds…) plus ~20 permissively-licensed ports (three.js MIT, pmndrs/postprocessing Zlib, drei-vanilla MIT, n8ao CC0) become `src/fx/<id>` modules with a metadata card: kind, mood/content tags, cost, dark/light, param schema, conflicts, source, licence. `deck3d fx list|preview` and a generated `reference/effects.md` let the agent pick by slide content; `parse` assigns deterministic defaults from content tags, `overrides` replace them. Non-permissive sources (LYGIA, Shadertoy) are inspiration only.
...
- `deck3d-effects`: curated, licence-clean effects corpus (backgrounds, post, materials, lights, motion, edge styles, transitions) with metadata cards, generated catalogue + previews, deterministic content-tag defaults the agent overrides, composition/budget rules, and a contribution procedure.
