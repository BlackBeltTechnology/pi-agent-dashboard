...
- [x] 5.5 **Verify, do not assume**, that `~/.pi/dashboard/local/token` is unreadable by a second OS user on Windows — `chmod` is a documented no-op there, so the guarantee rests on inherited NTFS ACLs
  ...
  - **RESOLVED 2026-09-14 by `verify-windows-credential-acls` — the handle was pulled, and the door held.** All three credentials returned `READ-DENIED` (`UnauthorizedAccessException`) from a real second STANDARD user on `windows-latest` (`ci-gateway-platform.yml` run 34823022229, 4m45s, PASS).
  - **The `infeasible` diagnosis above was WRONG, and it is worth being exact about why.** `Start-Process -Credential` on a hosted runner does NOT fail to give the second user a usable logon. The arm's own probe was writing its verdict into the session owner's `%TEMP%` while granting the second user only `(RX)` — and a standard user cannot traverse another user's profile at all, so even a SUCCESSFUL impersonation produced no output. `"produced no output"` was indistinguishable from a logon that never happened, and that ambiguity was misread here as a runner limitation. Moving the probe to `C:\ProgramData\qa-acl-<pid>` with `(M)` for the second user made the verdict observable on the first try. A null result was read as evidence about the environment when it was evidence about the test.
  - Cross-check that the denial came from the second user: the files are owned by `BUILTIN\Administrators`, so a read by the runner's own principal would have SUCCEEDED.
- [x] 5.6 If 5.5 fails, treat it as a pre-existing defect affecting `identity.key` and `paired-devices.json` too, and raise it as its own change rather than patching it here
  ...
  - **RESOLVED 2026-09-14: N/A — 5.5 did not fail.** Every read was DENIED, so there is no pre-existing defect to raise. The extended arm covers all three files and a broad-principal DACL is reported per file.
...
- [x] 12.52 **Deferred here; DELIVERED by `serve-retained-remote-transcripts`** as `tests/e2e/remote-transcript-read.spec.ts` ("history predating the attach is served, in order, and renders"), proven load-bearing by a revert run. New spec modelled on `tests/e2e/large-session-replay.spec.ts` — pre-attach history renders. Triple: remote session with entries predating the bridge attach · open the transcript · pre-attach entries present in the rendered transcript. (test-plan #F6)
...
- [x] 12.53 Windows credential readability on a real Windows host: log in as a second standard OS user and attempt to read `~/.pi/dashboard/local/token`, `identity.key`, and `paired-devices.json`. `chmod` is a documented no-op on Windows, so this rests on inherited NTFS ACLs and must be observed rather than asserted. (test-plan: manual-only)
  ...
  - **RESOLVED 2026-09-14 — impersonation is FEASIBLE on a hosted runner, so this task closes on CI evidence.** `verify-windows-credential-acls` observed `READ-DENIED` for all three credentials on `windows-latest` (run 34823022229). By this note's own rule the task now closes; the earlier `infeasible` was the probe's verdict file being unreachable by the second user, not an unavailable logon. The arm no longer has a pass-without-a-verdict path: a missing verdict is a hard FAIL. The `qa/` VM matrix was NOT used.
...
- [x] 13.5 Remote-join end-to-end: local pi pairs with a remote dashboard, registers, survives a reconnect, serves history predating the attach, retains that history after the session ends, and is locked out after revocation
  - **The struck clause is restored.** "Serves history predating the attach" was struck here, not ticked, because the READ path was deferred with §11: `RemoteTranscriptStore.read()` had no caller and no route. Change `serve-retained-remote-transcripts` shipped that path — a read route, remote-origin hydration, and `tests/e2e/remote-transcript-read.spec.ts`, whose first scenario registers a genuinely remote session, delivers a real `transcript_chunk`, and asserts the pre-attach entries RENDER. The clause is now true, so it stands unstruck.
