...
- [x] 8.6 **Spawn `DocScribe`** for `docs/` prose — why grants are a subtree check rather than an `isAllowed` anchor, and the realpath-at-grant-time rule. → verify: caveman-style rows returned and applied by the parent  [done: DocScribe wrote `### Access Grants and Denial Remedies` into docs/architecture.md (placed after the Network Access Control section, cross-referenced not re-explained) + the docs/AGENTS.md Gains clause + docs/architecture.md.AGENTS.md sidecar. Parent verified every load-bearing claim against source: `isAllowed` then `else if (await isGrantAdmitted(...))` (containment-gate.ts:72-74), store at getDashboardConfigDir()/access-grants.json cap 200/scope, forbidden-subjects realpath compare + `sensitive` exact-and-descendant rule, and the fd fix (file-routes.ts parses range at :876 BEFORE opening the verified handle at :1045, so the four `416` returns at :887-914 cannot leak).]
- [x] 8.7 **`eng-disciplines` → `review-code`** on the full diff before commit. → verify: findings resolved or consciously accepted  [done: review-code on the full diff, engine run cross-model (the `@review` role returned EMPTY — out of quota — so the pass ran on `@propose-review-2`, ZAI GLM, a different family from the author's deepseek-flash). Verdict: could not pass as-is. **2 issue(blocking), both reproduced and both VERIFIED by the parent in source, both fixed with regression tests that fail pre-fix (4 new cases: 4 failed pre-fix / 44 passed post-fix):** (1) the ancestor ladder's boundary guard nulled itself when the checkout root EQUALED the subject or its direct parent, so the climb ran past the repo and offered the directory holding every sibling repo — the exact one-click widening the module exists to prevent; (2) `DELETE /api/access/worktree-trust` was a silent no-op reporting success: the client sends no hash, `revokeTrust(subject, "")` deleted the literal `repoRoot\0` key that never exists, so the Access tab's revoke button revoked NOTHING while returning `{success:true}`. Also fixed 1 issue(non-blocking): the EML gates skipped the D14 handle guard the office gates had. Corrected a stale client header naming endpoints the code deliberately does not use. Remaining findings consciously accepted as trade-offs/nitpicks (double `isAllowed` on a miss; missing-path probes 403-vs-404 asymmetry; `session: cwd` mislabel; dashboard-dir derivation).]
- [x] 8.8 **`eng-disciplines` → `doubt-driven-review`** on the persisted store format — effectively irreversible once shipped. → verify: decision recorded in `design.md` if anything changes  [done: cross-model adversarial review on `@propose-review-2` (ZAI GLM) — `@propose-review-1` SKIPPED as same-family, author runs deepseek-flash. 12 findings; reconciled not rubber-stamped. 6 fixed (all load-path: forbidden-subject re-check at load, `version===1` refusal, cap-at-load, disk `session`-scope drop, finite `grantedAt`, `origin` coercion) + 1 write-path fix (`enforceCap` never evicts the grant being recorded — D11 truthfulness), each with a regression test that FAILS on the pre-fix code (verified: 8 failed / 21 passed pre-fix → 29 passed post-fix). 6 recorded as accepted trade-offs. Decision recorded in design.md as **D21**.]
