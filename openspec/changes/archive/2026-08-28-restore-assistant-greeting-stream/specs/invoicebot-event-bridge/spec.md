...
the declared list (nor the separately-declared greeting render channel) SHALL
NOT be forwarded (declared-set semantics; the former raw-name passthrough is
retired). The core `packages/extension` and `packages/server` code SHALL
contain no `ib:*`/`ib_*` channel knowledge.
