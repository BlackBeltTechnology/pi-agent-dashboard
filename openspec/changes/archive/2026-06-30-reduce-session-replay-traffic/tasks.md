...
- [x] 8.4 Strategy B AUTOMATED via Playwright E2E (`tests/e2e/replay-stub-expand.spec.ts`): full replay in a fresh browser context ships a wire stub `{stub,byteSize>4000,preview,entryId}` with no body. PASSES against Docker (stable, ~9s). The E2E surfaced TWO real bugs the deterministic tests missed (now fixed + regression-tested): (1) `maxStringFieldSize` defaults to 0 → truncation DISABLED in prod, so byteSize must not gate on a truncated copy; (2) live `result` is structured `{content:[{type:text}]}`, not a string. Fixes: `recordToolResultByteSize` computes byteSize from `extractToolResultText` independent of truncation + annotates a copy; route/stub handle both shapes; live key = toolCallId. Faux scenario `tool-bash-large` added (two-step, terminates).
