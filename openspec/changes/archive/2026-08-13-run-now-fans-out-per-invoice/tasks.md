...
- [x] 1.1 In `packages/automation-plugin/src/server/engine.ts`, extract the
  ...
  `contexts: []`).
- [x] 1.2 Rewire `dispatchFire` to consume the shared helper (behaviour
  unchanged: skip → drop the fire with a warn; empty → no fire; else
  `runner.fire` per context).

## 2. Fan-out-aware run-now

- [x] 2.1 Add `engine.runNow(automation)` to the Engine interface + return:
...
- [x] 2.2 Route `runNowViaEngine` in `index.ts` through `engine.runNow(found)`
  instead of `engine.startRunFor(found)`.

## 3. Tests

- [x] 3.1 Run-now on a per-invoice automation with a queue of N fans out to N
...
- [x] 3.2 Run-now on a per-invoice automation with an empty queue starts no run
  and returns `{ ok: true }` with no runId.
- [x] 3.3 Run-now on a per-invoice automation with no enumerator wired starts no
  run and returns `{ ok: false }`.
- [x] 3.4 Run-now on a non-per-invoice automation starts exactly one run
  (unchanged) and returns its runId.

## 4. Validate

- [x] 4.1 `openspec validate run-now-fans-out-per-invoice --strict` passes.
