...
- [x] 2.1 Add `diffAll` / `diffAllOr` to `packages/shared/src/platform/git.ts`: one `git diff -z HEAD` (no per-path arg) over the whole worktree, built on `runAsync` (async, non-blocking). Retain the existing sync `diffOr` for other callers.
- [x] 2.2 Add a splitter that parses the `-z` NUL-delimited unified-diff stream into a `Map<path, diffText>`, mirroring `gitNumstat`'s per-file keying. Handle rename/copy headers to the NEW path (consistent with `parsePorcelain`).
