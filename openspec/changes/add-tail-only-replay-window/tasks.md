...
- [x] 2.11 **DEVIATION (recorded):** moved the reset into `sendEventBatches`, emitted when `replayWindow !== null`, immediately before `history_window`, and deleted the WARM call-site guard. The stale-`lastSeq` guard is KEPT: it was never window-gated (it fires on `lastSeq > maxSeq` regardless) and a shipped test asserts it against a 3-event store where no window can apply. Deleting it would be a third behaviour change D3 does not enumerate. Cost: a windowed stale-`lastSeq` replay emits two idempotent resets, both before any `event_replay`. Original text: move the reset into `sendEventBatches`, emitted when `replayWindow !== null`, immediately before `history_window`; delete the two call-site guards. No asset-ordering contingency — `session_state_reset` reduces to `createInitialState()` and `asset_register` is a documented no-op in that reducer. Green 2.7–2.10.
