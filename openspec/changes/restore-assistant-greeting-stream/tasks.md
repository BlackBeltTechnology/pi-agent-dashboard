...
- [ ] 1.1 In `packages/server/src/ib-domain-event-cache.ts`, add a bounded,
...
- [ ] 1.2 Extend `clearForSession` to also clear the session's retained greeting
      stream; keep the latest-per-key clear behavior unchanged.
- [ ] 1.3 Ensure each retained greeting carries a stable id and a monotonic
      per-session ordering key (derive/attach if the incoming frame lacks one).

## 2. Server: intake + connect replay routing

- [ ] 2.1 In `packages/server/src/server.ts` at the `ibDomainEventCache.set(...)`
...
- [ ] 2.2 In `packages/server/src/pairing/browser-gateway.ts` at the connect-replay
...
- [ ] 3.1 In `packages/shared/src/browser-protocol.ts`, add the additive ordering
...
- [ ] 4.1 In `packages/client/src/lib/chat/event-reducer.ts`, add handling that
...
- [ ] 4.2 Add an additive optional `state` field to `ChatMessage`
      ...
      content.
- [ ] 4.3 In `packages/client/src/components/chat/ChatView.tsx`, add a
...
- [ ] 4.4 Confirm a producer-authored raw inline `<svg><path/></svg>` glyph in
      greeting content renders as real DOM through the existing `MarkdownContent`
      path (no code change expected).
- [ ] 4.5 Ensure greetings render only from the domain-event frame; confirm the
      `display:false` transcript copy still produces no row (reducer + replay path).

## 5. Tests

- [ ] 5.1 Server unit test: three greetings for one session replay in order on
      connect (not collapsed to newest), marked `replay: true`; session death
      clears them; per-session cap evicts oldest-first.
- [ ] 5.2 Server test: a non-greeting domain event still uses latest-per-key
      convergence (unchanged).
- [ ] 5.3 Reducer test: greeting frames fold into chat rows in chronological order
...
- [ ] 5.4 Fail-loud unit test: the folded greeting message carries the structured
...
- [ ] 5.5 Pin the raw-HTML contract: a test asserting `MarkdownContent` renders a
...
- [ ] 5.6 Run the gate: `HOME=$(mktemp -d) pnpm exec vitest run <touched test files>`
