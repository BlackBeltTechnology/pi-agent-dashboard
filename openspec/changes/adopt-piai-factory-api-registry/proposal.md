...
- Factory-path streaming calls the resolved runtime's own `normalizeContext` and then dispatches **api-first** (`model.api` → lazy api implementation), falling back to the owning built-in provider. It deliberately does **not** go through `Models.streamSimple`, which throws for OAuth-only providers such as `openai-codex` when handed a caller-supplied key. Nothing is registered into the built-in collection.
