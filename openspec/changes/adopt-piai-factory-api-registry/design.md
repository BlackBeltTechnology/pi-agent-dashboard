...
- **Dispatch is api-first, provider-second.** `InternalRegistry` deliberately retains a custom model authored under a built-in provider name. Resolving by provider first sends such a model into a built-in `Provider`, and `createProvider`'s single-api form ignores `model.api` entirely — streaming it through the wrong api. 0.75.5 dispatched purely on `model.api`; api-first preserves that. A model whose api has no entry **and** whose provider is unknown fails with a diagnosable error naming both.
...
- **`options.env` / provider-injected auth headers (Cloudflare class)** → `Models.applyAuth` supplies `env`; provider-level dispatch does not. Built-in models carry a baked-in `baseUrl`, and 0.75.5's global `streamSimple` likewise supplied no `env`, so this is expected parity rather than regression — but it is **asserted, not proven**, for the Cloudflare gateway providers. An implementation-time check against `providers/cloudflare-*.js` is required before claiming the "same upstream request" scenario for that class.
