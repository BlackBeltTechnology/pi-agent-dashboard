...
**D17. `models.json` writes are SERVER-OWNED, change-triggered. (Cross-model doubt finding, verified; RESOLVED server-owned.)** Every pi session runs the extension; a per-session-startup read-modify-write of `models.json` races (unlocked `loadModelsJson`→merge→`saveModelsJson`) and drops providers (Bug 5). RESOLUTION: the dashboard SERVER is the single writer. It already owns provider CRUD (`provider-routes.ts` writes `providers.json#providers` on UI add/edit) — discovery (`/v1/models` fetch + metadata enrichment) and the `models.json` write MOVE server-side, triggered by that same CRUD action. Every pi session (interactive/flow/subagent) then only READS pi-native `models.json` via `ModelRegistry.create` — no per-session write, no discovery, no lock, no concurrency. The extension keeps runtime `registerProvider` as an in-session fast-path and `preRegisterProviderAuth` for the newly-added window (D18), but is no longer a `models.json` writer. *Alt (extension-owned-with-lock):* rejected — needs a cross-session file lock and still duplicates discovery per session; server-owned removes the race class entirely.
...
- ~~Who owns `models.json` writes?~~ RESOLVED (D17): **server-owned** — discovery + write move server-side, triggered by provider-routes CRUD; sessions read-only.
