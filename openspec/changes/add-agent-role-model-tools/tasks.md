...
- [ ] 0.2b **RESOLVE D17 open question first**: who owns `models.json` writes — server-owned (move `/v1/models` discovery server-side, write on provider-routes CRUD) vs extension-owned-with-lock. Blocks 0.3.
- [ ] 0.3 Wire the writer as a SINGLE-OWNER, CHANGE-TRIGGERED write (per D17): persist on provider add/edit/remove only, NEVER per-session-startup (concurrent read-modify-write races → last-writer-wins data loss, cross-model Bug 5). **KEEP `preRegisterProviderAuth`** (D18/Bug 3): a newly-added provider is absent from `models.json` until its ~10s discovery completes, so the sync auth pre-registration still closes the spawn window. Runtime `registerProvider` stays as the in-session fast-path; `models.json` is the durable/cross-process source.
- [ ] 0.4 **Rework the SERVER reader (D16/Bug 1+2)**: `registry-singleton.ts readModels()` currently expects flat `raw.models` — change it (and `internal-registry.ts` composition) to parse pi-schema `{providers:{<name>:{baseUrl,api,models[]}}}`, sourcing each model's `baseUrl`/`api` from its provider entry (not from `providers.json`, which is empty post-migration). Verify `GET /api/models` returns custom-provider models with a non-empty `baseUrl`. Update `internal-registry` + `models-introspection-routes` tests. Also make the server's provider write path atomic (Bug 7).
