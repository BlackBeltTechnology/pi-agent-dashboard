...
- [x] 0.1 Add a `models.json` writer in the extension (`models-json-writer.ts`): pure `mergeManagedProviders(current, managed, managedNames)` (upsert managed, drop stale managed, preserve hand-authored + non-`providers` keys) + `loadModelsJson`/`saveModelsJson` (atomic tmp+rename) + `persistManagedProviders`. Dashboard-managed set tracked in `providers.json#managedProviders` (D15), `models.json` stays pure pi schema.
- [x] 0.2 Tests (`models-json-writer.test.ts`, 7 passing): merge upsert; hand-authored preserved; stale managed removed; non-`providers` keys preserved; IO roundtrip; tolerant read (missing/malformed). (Fresh-`ModelRegistry` load assertion moved to 0.4 where pi-ai/authStorage are available.)
- [x] 0.2b RESOLVED (D17): **server-owned** — `/v1/models` discovery + `models.json` write move server-side, triggered by provider-routes CRUD; pi sessions read-only. Unblocks 0.3/0.4.
- [ ] 0.3 Extension side (per D17): the extension is NO LONGER a `models.json` writer and no longer needs startup discovery — pi's `ModelRegistry.create` loads customs from server-written `models.json`. KEEP runtime `registerProvider` as the in-session fast-path AND `preRegisterProviderAuth` (D18/Bug 3: newly-added provider absent from `models.json` until server discovery completes). Remove the per-session `models.json` write path (the writer is invoked server-side, not here). `models-json-writer.ts` becomes a shared helper usable by the server.
- [ ] 0.4 **Server owns discovery + write + reader rework (D16/D17, Bug 1/2/7):** (a) on provider add/edit/remove (`provider-routes.ts`), the SERVER fetches `/v1/models`, enriches metadata, and writes `models.json` via the shared `persistManagedProviders` (atomic); (b) rework `registry-singleton.ts readModels()` + `internal-registry.ts` to parse pi-schema `{providers:{<name>:{baseUrl,api,models[]}}}`, sourcing each model's `baseUrl`/`api` from its provider entry; (c) make the server provider-config write atomic (tmp+rename, Bug 7). Verify `GET /api/models` returns customs with non-empty `baseUrl`. Update `internal-registry` + `models-introspection-routes` + `provider-routes*` tests.
- [ ] 0.4b Handle `managedProviders` empty/missing safely (Bug 8): an absent/empty set MUST NOT strand previously-managed providers in `models.json` forever — self-heal or reconcile. Add a test.
- [ ] 0.5 Write the auto-migration script (`scripts/migrate-providers-to-models-json.ts`), ORDERED write-verify-then-strip (D18/Bug 9): (1) read `providers.json#providers` → discover/enrich; (2) back up both files; (3) write `models.json` (merge, atomic) and VERIFY via a fresh `ModelRegistry` that the customs load; (4) ONLY then strip `providers` from `providers.json`, preserving `roles`/`rolePresets`/`activePreset`; on any failure restore from backup. Idempotent no-op on re-run.
- [ ] 0.6 Tests: migration moves providers + preserves roles + creates backups + fresh `ModelRegistry` loads customs; crash-before-strip leaves providers recoverable; second run is a no-op; apiKey round-trip incl. `$ENV_VAR`/`!command` + a literal key containing `$` (cross-model apiKey edge). Run full `npm test` — all pre-existing provider/model/resolution tests (`build-provider-catalogue`, `custom-provider-apikey-roundtrip`, `enrich-model-metadata`, `provider-register-reload`, `internal-registry`, `provider-routes*`, `models-introspection-routes`) MUST stay green.
