...
- [ ] 8.15 L2: resources refresh across 10 known directories · one poll cycle · p95 `GET /api/pi-resources` ≤ 5s warm and `resolve()` bounded by a 5s timeout — see `qa/tests/02-server-start.sh` (test-plan #P1)
...
- [ ] 8.30 L1: session with a populated retained set · a skill-less `commands_list` arrives mid-reload · retained set unchanged — a non-empty skill set is never replaced by an empty one (test-plan #X4)
