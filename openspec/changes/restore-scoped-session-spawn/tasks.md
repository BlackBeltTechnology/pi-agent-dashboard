...
- [ ] 2.2 Update `per-invoice-fanout.test.ts`: the per-invoice fan-out run keeps
  the automation's own name (no scoped-name injection).

## 3. Verify

- [ ] 3.1 Scoped gate: `automation-plugin` + `invoicebot-plugin` tests green.
- [ ] 3.2 `npm run build` + tsc clean.
- [ ] 3.3 QA — full fresh e2e retest (driven by the orchestrator after this and
  the ui change merge).
