...
- [ ] 1.1 Build `develop` and record cold-landing **root JS transfer bytes** under
...
- [ ] 1.2 Record the LCP for the same load as the "before" evidence row; verify it
      is written below beside the baseline
- [ ] 1.3 Record which chunks `dist/index.html` preloads and stylesheet-links
      today; verify `xterm-*` and `diff-*` appear in **both** lists (this is the
      property the change inverts)

Baseline (fill in 1.1/1.2): root JS transfer = `____ KB` · LCP = `____ s`

## 2. Chunk split prerequisite (design D1)

- [ ] 2.1 Split npm `diff` out of the `diff` manual chunk in
...
- [ ] 2.2 L1 test — npm `diff` and `@git-diff-view/*` land in different emitted
      ...
      chunks (test-plan #E5)
- [ ] 2.3 L1 test — chat-path jsdiff does not reach the diff viewer · see
      ...
      (test-plan #E8)
- [ ] 2.4 L1 test — build emits no circular-chunk warning · see
...
- [ ] 3.1 L1 test — single mount per id within a pane · see
...
- [ ] 3.2 L1 test — keep-alive across tab switch · see
      `packages/client/src/components/editor-pane/__tests__/EditorPane.test.tsx`
...
- [ ] 3.3 L1 test — unmount on close · same exemplar · Triple: pane with a single
      terminal tab `t1`, layer mounted · close the `t1` tab · `TerminalView` for
      `t1` is unmounted (test-plan #F7)
- [ ] 3.4 Verify 3.1–3.3 pass against unmodified `develop`; a test that is red
      before the gate change is testing the wrong thing

## 4. Terminal lazy boundary + activation latch (design D3, D3a)

- [ ] 4.1 Add the sticky activation latch to the **`SplitWorkspaceContext`
      ...
      still pass
- [ ] 4.2 Make `TerminalPaneLayer` a `React.lazy` import in `EditorPane.tsx`,
      ...
      unlatched
- [ ] 4.3 Pass `activate: false` on the auto-surface dispatch in
...
- [ ] 4.4 Make `InlineTerminalCard` lazy in `ChatView.tsx` with a card-local
      `Suspense`; verify the transcript stays scrollable while the card loads
- [ ] 4.5 L3 test — no terminal/diff chunk on a clean cold landing · see
...
- [ ] 4.6 L3 test — restored **background** terminal tab does not load terminal
      ...
      (test-plan #F2)
- [ ] 4.7 L3 test — folder auto-surface opens in background · see
      ...
      (test-plan #F3)
- [ ] 4.8 L3 test — latch fires on activation · same exemplar · Triple: state of
...
- [ ] 4.11 L3 test — fallback fills the pane body · see
...
- [ ] 4.12 L1 test — dead persisted tab is closed · see
...
- [ ] 4.13 L1 test — cold-load snapshot race preserved · same exemplar · Triple:
...
- [ ] 5.1 Make `FileDiffView` lazy in `App.tsx` with its own `Suspense` at the
      route render site (`:2157`); verify the diff route renders
- [ ] 5.2 Give `shellRenderers.renderDiff` (`App.tsx:2493`) its **own** local
      `Suspense` inside the returned JSX; verify no suspension escapes to the
      shell
- [ ] 5.3 Make the `diff` pseudo-tab entry in `pseudo-tab-registry.tsx` a lazy
      wrapper; add **no** new `Suspense` — `EditorPane.tsx:176` already has one
- [ ] 5.4 Make `RichDiff` lazy in `EditToolRenderer.tsx` with a renderer-local
      `Suspense`; verify the mobile `HomegrownDiff` path is untouched
- [ ] 5.5 L1 test — viewer-registry partition holds under the lazy entry · see
      ...
      (test-plan #E9)
- [ ] 5.6 L1 test — pseudo-tab key resolves · see
...
- [ ] 5.7 L1 test — `renderDiff` callback contains its own suspension · see
      `packages/client/src/components/editor-pane/__tests__/EditorPane.test.tsx`
...
- [ ] 5.8 L3 test — diff route loads and the shell survives the wait · see
...
- [ ] 6.1 Add `packages/client/src/__tests__/lazy-feature-preload.test.ts` · see
      `packages/client/src/__tests__/eml-bundle-exclusion.test.ts` · Triple:
...
- [ ] 6.2 L1 test — stylesheet arm · same file · Triple: production build · parse
      every `<link rel="stylesheet" href>` · no basename matches `/^xterm-/` or
      `/^diff-/`; verify both ARE linked pre-change (test-plan #E2)
- [ ] 6.3 L1 test — existence backstop, JS **and** CSS · same file · Triple:
      ...
      (test-plan #E3)
- [ ] 6.4 L1 test — anchored matcher survives the D1 split · same file · Triple:
      build after npm `diff` moves to the `jsdiff` chunk key · run 6.1/6.2 ·
      `jsdiff-<hash>.js` does not trip the `/^diff-/` matcher (test-plan #E4)
- [ ] 6.5 Make the guard skip cleanly when `dist/` is absent; verify `npm test`
      passes on a clean checkout with no build present
- [ ] 6.6 L3 test — scoped carve-out is pinned, not assumed · see
...
- [ ] 7.1 L3 test — chunk fetch aborted · see `tests/e2e/csp.spec.ts` for
...
- [ ] 8.1 L3 test — root JS transfer budget · see
...
- [ ] 8.2 Re-measure LCP per 1.2 after the change and record the "after" row
...
- [ ] 9.1 Convert existing synchronous render assertions that now hit a Suspense
      ...
      removing a boundary
- [ ] 9.2 Manual smoke on the running dashboard: open a terminal tab, an inline
