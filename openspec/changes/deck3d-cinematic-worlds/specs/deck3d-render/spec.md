...
#### Scenario: Spinning topologies keep their captions readable
- **WHEN** a topology that rotates its group (`globe`, `loop`, `swarm`, `orbit-cluster`) plays
- **THEN** every caption SHALL keep facing the viewer as the group turns, instead of going edge-on and then away
