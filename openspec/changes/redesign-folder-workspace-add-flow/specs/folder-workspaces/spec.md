...
### Requirement: Compact add-to-workspace affordance

The add-to-workspace gesture SHALL be presented as a compact icon button composed of `mdiFolderPlus` plus a
trailing `mdiMenuDown` disclosure caret, sized to match the sibling icon buttons in the row's action cluster
(nominal 28×20 px, padded to a ≥44 px touch target at mobile breakpoints). It SHALL carry an accessible name
conveying the full verb (e.g. "Add to workspace…") via `aria-label` and `title`, and SHALL expose
`aria-haspopup` with `aria-expanded` reflecting popover state. The prior 10px `+ws` text token, positioned in
its own absolutely-placed layer outside the cluster, SHALL be removed.
...
#### Scenario: Folder row renders the icon button, not the text token

- **WHEN** a top-level folder group header renders and at least one workspace exists or workspace creation is available
- **THEN** an icon button with an accessible name for adding to a workspace SHALL render inside the header's icon cluster
...
- **THEN** an icon button with an accessible name for adding the session's cwd to a workspace SHALL render in the card's header cluster
