...
- [ ] 1.2 Confirm the red baseline before touching source: the §5 e2e probes must fail
...
- [ ] 2.1 Structured runtime error, nominal shape (test-plan #E1). Input: `isError=true`,
...
- [ ] 2.7 Banner strip × new fields (test-plan #E7). Input: `⚠️ context-mode v… outdated …`
...
- [ ] 2.8 Fence with no language (test-plan #E8). Input: bare ``` fence + `Exit code: 1` ·
      Trigger: parser runs · Observable: `command` set, `language` `undefined`.
- [ ] 2.9 Delimiter injection (test-plan #E9). Input: a `stdout:` section whose own text
...
- [ ] 2.10 Multi-fence body (test-plan #E10). Input: two fenced blocks + an exit line ·
...
- [ ] 2.11 Malformed-input fuzz table (test-plan #X1). Input: empty string, only a fence,
...
- [ ] 3.1 Structured card renders sections (test-plan #F1). Input: #E1's parsed struct ·
...
- [ ] 3.2 Chrome-vs-content split (test-plan #F2). Input: any multi-line ctx error card ·
...
- [ ] 3.3 Error signal redundancy (test-plan #F3). Input: neutral-body error card ·
...
- [ ] 3.4 Five single-line surfaces (test-plan #F4). Input: `exit 3` bash badge ·
...
- [ ] 3.5 receivedArgs block unchanged (test-plan #F5). Input: validation error with
      `receivedArgs` · Trigger: render + expand · Observable: still `--text-secondary` on
      `--bg-code`; no severity class added by this change.
- [ ] 3.6 Unstructured fallback renders flat (test-plan #F6). Input: #E5's parsed struct ·
...
- [ ] 3.7 Partial extraction degrades (test-plan #X2). Input: struct with `command` set but
...
- [ ] 3.8 Oversized stream inherits truncation (test-plan #X3). Input: runtime error with a
...
- [ ] 4.1 Resolve design Q1: guard in `scripts/check-conventions.mjs` (ship gate) vs a
      Biome rule (`quality:changed`). Record the choice in `design.md`.
- [ ] 4.2 Guard decision table (test-plan #E11). Input: (a) `text-red-300` in
      ...
      escape hatch.
- [ ] 4.3 Allowlist integrity (test-plan #E12). Input: the ~40 pre-existing raw red
...
- [ ] 5.1 Seven surfaces clear the floor (test-plan #F7). Input: probes built from the
...
- [ ] 5.2 Links legible on `--bg-code` (test-plan #F8). Input: a `LinkifiedText` link
      rendered on `--bg-code` · Trigger: 18 theme·mode sweep · Observable: link colour vs
      resolved `--bg-code` ≥ 3:1 in every combo.
- [ ] 5.3 Anti-vacuity guard (test-plan #F9). Input: the same probes as #F7 · Trigger: read
...
- [ ] 6.1 `CtxToolRenderer.tsx:187` — container to
      `bg-[var(--severity-error-bg)] border-[var(--severity-error-border)]`.
- [ ] 6.2 `:188` — label to `text-[var(--severity-error-fg)]`.
- [ ] 6.3 `:189` — body to `text-[var(--text-secondary)] p-2 bg-[var(--bg-code)] rounded`,
...
- [ ] 6.4 Extend the `runtime` arm of `parseError` to extract
      `{command, language, exitCode, stdout, stderr}`. Keep it pure and React-free.
- [ ] 6.5 `CtxToolRenderer` error card — when structured, render `command` through the
...
- [ ] 6.6 Unstructured runtime error falls back to the flat neutral body from 6.3. No text
      dropped.

## 7. Implementation — the five single-line surfaces

- [ ] 7.1 `ToolBurstGroup.tsx:383` — `N failed` badge to severity bg/fg/border.
- [ ] 7.2 `BashOutputCard.tsx:46` — non-zero `exit N` badge to severity bg/fg. Leave the
      success branch (`green-500/20`) alone; it is out of scope and a separate tier.
- [ ] 7.3 `ToolCallStep.tsx:149` — errored status icon to `--severity-error-fg`. Leave the
      sibling `sky-400` / `green-400` / `yellow-400` branches alone.
- [ ] 7.4 `AskUserToolRenderer.tsx:220-221` — icon to `--severity-error-fg`; message to
      `--text-secondary`. Drop the `/80` alpha: it compounds a low-contrast literal and has
      no purpose once the token is derived.
- [ ] 7.5 `AgentToolRenderer.tsx:346` — `Error:` marker to `--severity-error-fg`, the
      message itself to `--text-secondary`.

## 8. Implementation — make the regression impossible

- [ ] 8.1 Implement the guard over the governed file allowlist only (per the 4.1 decision).

## 9. Verify

- [ ] 9.1 All §2–§5 scenarios green.
- [ ] 9.2 `npm test` green; report the red list before touching source if any.
- [ ] 9.3 Biome + `tsc --noEmit` clean (`npm run quality:changed`).
- [ ] 9.4 `review-code` pass on the diff — six components, two directories.
- [ ] 9.5 `doubt-driven-review` on the chrome-vs-content rule before it becomes precedent
      for every future error surface.
- [ ] 9.6 Deploy: client-only change → `npm run build` + `POST /api/restart`.
