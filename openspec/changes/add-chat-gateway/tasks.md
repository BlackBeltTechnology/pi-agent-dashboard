...
- [x] 1.1 Create `packages/chat-gateway/` (server component + settings panel) as a dashboard plugin discovered by `dashboard-plugin-runtime`. → verify: plugin appears in `/api/health.plugins[]`.
- [x] 1.2 Add `discord.js` as an adapter-local dependency. → verify: builds; no new dep leaks into core packages.
- [x] 1.3 Inert-when-unconfigured guard (no token → no connection). → verify: with no token, `npm test` + dashboard start show zero gateway activity.

## 2. Vendor the Discord adapter (MIT)
- [x] 2.1 Vendor `base.ts` (`PlatformAdapter` contract) + `discord.ts` into `src/adapters/` with a `NOTICE` attributing `@gamalan/pi-gateway` (MIT). → verify: `NOTICE` present; no `index.ts`/`sessions/store.ts`/`ask-user-rpc` copied.
- [x] 2.2 Adapter carries no hub logic (no spawn, no `pi --mode rpc`, no HTTP/WS server). → verify: grep the adapter for `spawn`/`--mode rpc`/`createServer` returns nothing.

## 3. Headless client seam
- [x] 3.1 Implement the client that speaks `subscribe`/`send_prompt`/`prompt_response`/`abort` and consumes `event`/`event_replay`/`prompt_request`/`prompt_dismiss` (in-process subscribe if `dashboard-plugin-runtime` exposes it, else loopback WS). → verify: gateway receives events for a subscribed session.
- [x] 3.2 Assert zero changes to `packages/extension` (bridge) and the session event protocol. → verify: `git diff --stat` touches no bridge/protocol files.
...
- [x] 5.2 `allowedRoots` mandatory whitelist; reject any cwd outside it; refuse spawn when empty. → verify: path outside allowedRoots → no spawn + in-channel refusal.
- [x] 5.3 Attach-to-existing via `GET /api/sessions` (choose a live session in allowedRoots). → verify: attaches without spawning.
- [x] 5.4 Spawn correlation by cwd + recency from `session_register` (spawn returns no id). → verify: correlated sessionId matches the spawned session.

## 6. Inbound/outbound streaming
- [x] 6.1 Inbound authorized message → `send_prompt { text, delivery }`. → verify: prompt reaches bound session.
- [x] 6.2 Outbound: text deltas → throttled `editMessage` (single message edited in place) + typing indicator on turn activity. → verify: N deltas produce 1 edited message, not N posts.
- [x] 6.3 Disconnected session (502) → in-channel error, not silent. → verify: unreachable session replies with an error.
...
- [x] 8.1 L1 allowlist + pairing code; non-allowlisted messages never delivered. → verify: unlisted user → no send_prompt.
- [x] 8.2 L2 admin-only channel→cwd binding. → verify: non-admin bind refused.
- [x] 8.3 L4 DM isolation; group channels opt-in. → verify: group channel inert unless opted in.

## 9. L3 hard tool guard (companion extension)
- [x] 9.1 Companion in-session `tool_call` interceptor loaded into gateway-**spawned** sessions; deny-first policy → `{block:true}`. → verify: denied tool blocked before execution.
- [x] 9.2 Approval escalation via `ctx.ui.confirm` → PromptBus → Discord yes/no; tool proceeds only on approval. → verify: risky tool prompts in-channel; deny blocks, allow runs.
...
- [x] 12.1 allowedRoots containment: in-range accepted, out-range rejected, empty-set refused, **real-path symlink/`..` escape rejected**. → E1, E2, E3, E4.
- [x] 12.2 Binding sources & resolver precedence (persisted > fixedMap > default > interactive); attach in-range vs out-range. → E5, E6, E7.
- [x] 12.3 Spawn correlation via token; concurrent same-cwd spawns never cross-bind. → E8, E9.
- [x] 12.4 Sticky routing: 2nd message reuses sessionId; new thread → independent binding. → E10, E11.
- [x] 12.5 Auth decision table (allowlisted × admin × action); pairing TTL 15m / lockout 10. → E12, E13, X10.
- [x] 12.6 Config secrecy: token absent from read-back. → E14.
- [x] 12.7 Guard: denied tool `{block:true}`; attached session ungated; unanswered approval **fails closed**. → X2, X5, X4(logic).
- [x] 12.8 Mid-stream delivery mapping: plain → `followUp`, `!`-prefix → `steer`. → X6.
