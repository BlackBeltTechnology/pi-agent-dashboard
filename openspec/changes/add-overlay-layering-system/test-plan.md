...
| F3 | Overlay paints above `isolate` sibling cards (no underlap) | state-convergence | L3 | automated | dashboard with ≥2 session cards under a folder header; each card is `relative isolate` | open the folder-actions menu so its box overlaps a card below | `document.elementFromPoint(x,y)` at a point inside BOTH the menu bbox and a card bbox returns a node contained by `folder-actions-menu-panel-*` (menu is topmost) |
