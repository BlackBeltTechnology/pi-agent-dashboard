...
- [ ] 13.6 QA: Windows 11 24H2 ZIP cold launch. Expected: `~/.pi-dashboard/.version` written, `<managedDir>/node_modules/@blackbelt-technology/pi-dashboard-server/src/cli.ts` exists, server reaches `bootstrap.status=ready`, main window opens. The `'wmic' is not recognized` stderr line on Win 11 24H2 is cosmetic (process-scanner already falls back to tasklist/PowerShell) and is tracked separately from this change.
