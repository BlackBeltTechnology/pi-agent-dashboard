...
- [ ] 1.1 Confirm shipped code already satisfies every clause the delta adds:
...
- [ ] 2.1 Free-floating ask enters the panel (test-plan #E1). Input: pending
...
- [ ] 2.2 Tool-paired ask stays out of the panel (test-plan #E2). Input:
...
- [ ] 2.3 Widget-bar ask never enters the panel (test-plan #E3). Input: pending
...
- [ ] 2.4 Widget-bar precedence beats tool-pairing (test-plan #E4). Input:
...
- [ ] 2.5 Unregistered component type is not widget-bar (test-plan #E5). Input:
...
- [ ] 2.6 Tool-paired batch ask does not enter the stack (test-plan #E8). Input:
...
- [ ] 3.1 Single pending ask renders a one-card panel (test-plan #E6). Input:
...
- [ ] 3.2 Empty set hides the panel (test-plan #E7). Input:
  ...
  duplicating.
- [ ] 3.3 Late arrival appends without disturbing the incumbent (test-plan #F1).
  Input: panel rendered with `[p1]` · Trigger: rerender with `[p1, p2]` ·
  Observable: two `multi-ask-card-*` nodes, `p1` still present.
- [ ] 3.4 Drain to empty hides the panel (test-plan #F2). Input: panel rendered
  with `[p1, p2]` · Trigger: rerender with `[]` · Observable: no
  `multi-ask-panel` node.
- [ ] 3.5 Placement is stable with respect to the pending set (test-plan #F3).
...
- [ ] 3.6 Answering one card resolves only its own id (test-plan #F4). Input:
...
- [ ] 4.1 `review-code` pass on the added tests and the spec delta before
