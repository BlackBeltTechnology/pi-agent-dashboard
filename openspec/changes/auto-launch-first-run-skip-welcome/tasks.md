...
- [ ] 7.2 Local build + launch. Verify:
  - [ ] Splash appears, status flows `Starting → Checking → Launching → Opening`. (attach path skips launch-server splash steps; needs spawn path)
  ...
  - [ ] First-run marker `~/.pi/dashboard/first-run-done` exists post-launch. (attach path skips marker write; needs free `:8000` → spawn path)
...
- [ ] 7.6 **[doubt-review #1]** Verify bundled bridge registration still fires on a fresh install (marker absent): launch → a pi session in the workspace forwards events to the dashboard (confirms `registerBundledBridgeExtension()` survived the arm deletion).
