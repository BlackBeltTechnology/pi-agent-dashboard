...
- [ ] 1.1 In `scheduler.test.ts` (or `schedule-trigger` unit), add a fake-timer test:
...
- [ ] 1.2 Add a late-arrival test: arm a long delay, advance the fake clock **past
...
- [ ] 2.1 Decide fix location: **shared `setTimer` in `scheduler.ts`** (preferred —
...
- [ ] 2.2 Implement the chunked long-timeout: `MAX_DELAY = 2_147_483_647`. When the
...
- [ ] 2.3 Ensure the returned `Disposable.clear()` clears whichever hop is currently
  pending (no leaked inner timer after dispose mid-hop).

## 3. Verify

- [ ] 3.1 Tests from §1 now GREEN: long-delay cron fires **once** at the intended
  occurrence; no immediate-fire storm; late-arrival fires once at delay 0.
- [ ] 3.2 Existing short-cadence scheduler tests still pass (no regression on
  ≤ 24.8-day delays — the common path).
- [ ] 3.3 Confirm restart-skip is unchanged: a fresh `arm()` at `now` past a due
  occurrence returns the **next** future fire via `nextFire`, does not backfill.
- [ ] 3.4 `set -o pipefail; npm test 2>&1 | tee /tmp/pi-test.log` then grep the
  summary; automation-plugin suite green.

## 4. Rebuild

- [ ] 4.1 Server-side plugin change → `curl -X POST http://localhost:8000/api/restart`
  (jiti, no build). Confirm `/api/health` mode, then arm a short test cron to smoke
  the live path.

## 5. Spec

- [ ] 5.1 Add the "Armed timer honors delays beyond the 32-bit ceiling" requirement
