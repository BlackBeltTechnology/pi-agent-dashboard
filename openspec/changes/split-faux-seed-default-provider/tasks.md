...
- [ ] 2.2 L1 — file-wrapper fault cases in the same test file (tmp-dir FS):
...
- [ ] 2.3 L3 — extend an existing faux roundtrip spec (see `tests/e2e/faux-text.spec.ts` for the docker-harness + derived-port pattern; read `dashboardPort` from `.pi-test-harness.json`, never hardcode `:18000`):
...
- [ ] 3.1 Add a `scripts/AGENTS.md` row for `seed-settings-default-model.mjs` (purpose + `mergeDefaultModel`/`seedFile` exports + `See change:`), path-alphabetical.
- [ ] 3.2 Update the `docker/AGENTS.md` row for `test-entrypoint.sh` (settings seed now delegates to `scripts/seed-settings-default-model.mjs`; `See change:`).

## 4. Validate

- [ ] 4.1 `npm test` green (new L1 rows pass); `openspec validate split-faux-seed-default-provider --strict` clean.
- [ ] 4.2 Run the docker e2e harness for the faux roundtrip (I1) per `run-dashboard-e2e-local-changes` so it reflects the local seed change.
