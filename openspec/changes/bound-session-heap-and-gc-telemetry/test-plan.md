...
| E25 | heap-limits: server coupling guard arithmetic | BVA | L1 | automated | `maxTotalEventBytes` of `0`, `768` MiB, `2048` MiB against a `1536` MB ceiling | compute the budget's heap-equivalent against the ceiling | `0` and `2048` warn, `768` is silent; the reported figure is the heap-equivalent, not the raw budget |
| E26 | settings-panel: server coupling warning rendered | decision-table | L3 | automated | `maxTotalEventBytes` set to `0` against the default `1536` ceiling | blur the field | a non-blocking warning names the heap-equivalent; the value stays saveable |
...
| E30 | heap-limits: dashboard terminals do not inherit the ceiling | EP | L1 | automated | server running under a stamped ceiling, plus an operator-set heap flag | build a dashboard terminal environment | the server's old-space flag is absent; the operator-set flag is preserved |
...
| E27 | server-launch: Electron path is stamped | EP | L1 | automated | Electron shell launching the server via `launchDashboardServer` | build the launch invocation | the invocation carries the configured ceiling; it does not fall back to the runtime default |
...
| X16 | heap-limits: lowered default cannot ship without the byte bound | fault-injection (invariant) | L1 | automated | shared server-heap default below `8192` while the shared memory-limits default carries no `maxTotalEventBytes` | evaluate the shared invariant assertion | the assertion fails, failing the build |
