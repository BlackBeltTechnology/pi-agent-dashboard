...
`sessionHeap` SHALL accept `maxOldSpaceMb` (default `1024`), `initialOldSpaceMb`
...
- **THEN** `loadConfig()` SHALL return `sessionHeap.maxOldSpaceMb` of `1024` and `serverHeap.maxOldSpaceMb` of `8192`
...
- **THEN** `loadConfig()` SHALL return `maxSemiSpaceMb` of `8` and `maxOldSpaceMb` of `1024`
...
- **THEN** `loadConfig()` SHALL return the `1024` default
