...
- [ ] 3.10 Implement disarm (any `observe`+ principal from chat; only the dashboard re-arms) and verify both directions plus that mirroring continues while disarmed — **UN-MARKED (was [x], false): the CHAT half does not exist.** `TeamController.disarm()` has zero production callers (only `gateway-team-controls.test.ts` and `controller.test.ts` call it directly); `gateway.ts` parses no disarm command, while the dashboard RE-ARM path is wired in `index.ts`. Dashboard-side disarm/re-arm and X15 (mirroring survives disarm) ARE real. Needs: parse + authorize a disarm command at `observe`+ on a bound channel and call `team.disarm()`.
...
- [ ] 11.2 **Per-thread mirror level.** Authorization now inherits the parent binding (fixed in 804234c21), but the mirror LANE resolves its channel from the binding-store entry (`store.get(channelKey).channelId`) and `Binding` carries no `parentChannelId` — so a session spawned INSIDE a thread mirrors at the default level instead of the binding's, violating `chat-gateway-output-filter/spec.md:30` ("an operator switches a thread from names-only to full transcript"). Needs a `Binding.parentChannelId` field, its persistence, and the mirror call site to pass it.
