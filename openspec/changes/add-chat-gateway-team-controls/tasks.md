...
- [ ] 11.3 **The disarm latch is in-memory, so a server RESTART re-arms the layer.** The controller initialises `disarmed` from `config.disarmed` at construction, and a chat disarm never writes config, so a bounce clears it. The spec says the layer stays disarmed "until an operator re-arms it from the dashboard" — a restart is not an operator. Closing it needs either a gateway-owned state file (the `channels.json` / `command-log.json` pattern) or having the disarm write config, which changes the documented "dashboard is the only config writer" model. A design choice, not a patch.

- [ ] 11.4 **The attach path can bind a session outside the bound workspace.** When `resolveBindCwd` cannot resolve from workspace folders (all inert), it falls through to `fixedMap`/`defaultCwd` and finally the ATTACH path, which adopts the single live session anywhere inside `allowedRoots` with no check that its cwd is inside the *bound workspace*; the `spawn_session` grant also carries no `targetCwd`, so the chokepoint cannot catch it. Requires operator misconfiguration (workspace folders outside `allowedRoots`), and the NEXT message then correctly refuses `scope_violation` — but the binding, the subscription, and the mirror leak are already established. Found by the round-2 review.
- [ ] 11.5 **The disarm latch is global while principals are per-binding.** An `observe` principal mapped only on workspace A's channel halts action requests for EVERY workspace. Literally consistent with the spec ("disarm the layer"), but in tension with the requirement that a principal mapped for one binding gains no tier in another. Wants an explicit decision, not an emergent one. Found by the round-2 review.
- [ ] 11.6 **`audit.ts` rewrites the whole log synchronously on every append** (`writeFileSync` + `renameSync` + `chmodSync` per chat message, multi-MB at the default 10k retention), and a throw (ENOSPC/EACCES) propagates out of `authorizeRequest` leaving the user with no reply. `isEntry` also does not validate `outcome`, so a hand-edited file can load entries with no outcome. Found by the round-2 review.
