...
- [x] 2.1 Add the team-controls modules to chat-gateway's package (no second plugin, no second gateway connection) and verify the plugin still activates with one gateway connection and one routing table
...
- [x] 3.10 Implement disarm (any `observe`+ principal from chat; only the dashboard re-arms) and verify both directions plus that mirroring continues while disarmed — **UN-MARKED (was [x], false): the CHAT half does not exist.** `TeamController.disarm()` has zero production callers (only `gateway-team-controls.test.ts` and `controller.test.ts` call it directly); `gateway.ts` parses no disarm command, while the dashboard RE-ARM path is wired in `index.ts`. Dashboard-side disarm/re-arm and X15 (mirroring survives disarm) ARE real. Needs: parse + authorize a disarm command at `observe`+ on a bound channel and call `team.disarm()`. **NOW IMPLEMENTED:** `!disarm` is parsed in `handleInbound`, authorized by the SAME chokepoint as the verb `disarm` (`observe`+, via the new `CHAT_LOCAL_VERB_TIERS` map — it has no MCP row to read a tier from), then flips the latch. The `disarmChanged` comparison in `index.ts` was ALSO wrong and is fixed in the same commit: it compared against config, which a chat disarm never writes, so the dashboard re-arm read as "no change" and the layer could never be re-armed. Covered by 3 tests in `gateway-team-controls.test.ts` + `shouldApplyDisarm` in `index.test.ts`.
...
- [x] 7.1 Attach provenance via `ctx.assignSessionRef` (plugin-owned ref, not the user-curated tag namespace) carrying principal + channel + binding, and verify it persists across a dashboard restart and is absent for dashboard-only sessions
- [x] 7.2 Implement the append-only command log (principal, channel, thread, tier, verb, target, outcome, refusal reason) with no edit or delete interface and verify one entry per action-bearing request, permitted or refused
- [x] 7.3 Verify mirroring produces no log entries and that each refusal class records its own distinct reason
...
- [x] 10a.1 Test identifier+role max-wins: principal listed `observe` by id and holding a role mapped `control`, ceiling `operate` · authorize a `send_prompt` · grant tier is `control` (test-plan #E1; see `plugin-action-handler.test.ts`)
- [x] 10a.2 Test ceiling clamp: principal mapped `operate`, ceiling `control` · authorize any verb · grant tier is `control`, never `operate` (test-plan #E2; see `plugin-action-handler.test.ts`)
- [x] 10a.3 Test default ceiling: config with no ceiling key · authorize a `control`-tier verb for an `operate`-mapped principal · tier is `observe` and the verb refuses `insufficient_tier` (test-plan #E3; see `plugin-action-handler.test.ts`)
- [x] 10a.4 Test per-binding scoping: principal mapped `control` on binding A only · authorize in binding B · `Refusal{no_principal_mapping}` (test-plan #E4; see `plugin-action-handler.test.ts`)
- [x] 10a.5 Test fail-closed fresh binding: new binding with zero principals while another binding holds 3 at `operate` · authorize from each of those 3 in the new binding · all three refuse `no_principal_mapping` (test-plan #E5; see `plugin-action-handler.test.ts`)
- [x] 10a.6 Test all seven refusal reasons: one fixture per reason · authorize each · 7 distinct reason codes, exactly one per fixture (test-plan #E6; see `plugin-action-handler.test.ts`)
- [x] 10a.7 Test bot author outranks a mapping: `author.bot=true` with an id equal to a principal mapped `operate` · authorize · `Refusal{non_human_author}` (test-plan #E7; see `plugin-action-handler.test.ts`)
- [x] 10a.8 Test webhook author: message carrying `webhook_id` and no `author.bot` · authorize · `Refusal{non_human_author}` (test-plan #E8; see `plugin-action-handler.test.ts`)
- [x] 10a.9 Test role→`operate` rejected: config mapping a role to `operate` · schema validation · rejected with the operate-requires-identifier reason and not persisted (test-plan #E9; see `config-redact.test.ts`)
- [x] 10a.10 Test verb tier is read not declared: stub `GENERATED_TOOLS` row for `resume_session` altered to `operate` · authorize `resume_session` as `control` · `Refusal{insufficient_tier}` (test-plan #E10; see `plugin-action-handler.test.ts`)
- [x] 10a.11 Test verb outside the curated allowlist: `operate` principal, verb `shutdown_server` · authorize · refused despite sufficient tier (test-plan #E11; see `plugin-action-handler.test.ts`)
- [x] 10a.12 Test verb absent from the tier table: allowlisted command naming a verb with no `GENERATED_TOOLS` row · authorize · refused with no tier inferred (test-plan #E12; see `plugin-action-handler.test.ts`)
- [x] 10a.13 Test the NON_DELEGABLE four at `operate`: principal at `operate`, ceiling `operate` · each of `mint_device_token`, `set_providers`, `install_package`, `tunnel_connect` · all four refuse `non_delegable_verb` (test-plan #E13; see `plugin-action-handler.test.ts`)
...
- [x] 10a.16 Test disarm then re-arm: `observe` principal disarms · `control` principal attempts chat re-arm, then the dashboard re-arms · chat refused and state persists, dashboard restores action handling (test-plan #X14; see `plugin-action-handler.test.ts`) — **UN-MARKED (was [x], false): the flow cannot be tested because the chat disarm command it exercises was never implemented (see 3.10).** Its cited evidence, `plugin-action-handler.test.ts`, does not exist in packages/chat-gateway (it lives in other packages). The re-arm-refusal half IS covered: `controller.test.ts` asserts `rearmFromChat()` refuses with `rearm_requires_dashboard`. **NOW COVERED:** the chat-disarm half exists (task 3.10), and `gateway-team-controls.test.ts` "3.10: an OBSERVE principal disarms from chat..." drives the full flow at L1 — observe disarms, every principal is refused, chat re-arm is refused, the dashboard re-arms and the layer works again.
- [x] 10a.17 Test mirroring survives disarm: layer disarmed · bound session emits events · events mirrored while every action-bearing request refuses (test-plan #X15; see `gateway-team-controls.test.ts` "X15: mirroring survives disarm..." (the cited `plugin-action-handler.test.ts` is stale — it does not exist in this package))
...
- [x] 10b.1 Test sibling name prefix: ws folder `/a/fo`, session cwd `/a/foo` · resolve · no match (test-plan #E14; see `preferences-store.test.ts`)
- [x] 10b.2 Test nested folders across workspaces: ws A `/a`, ws B `/a/b`, cwd `/a/b/c` · resolve · ws B wins on longest match (test-plan #E15; see `preferences-store.test.ts`)
...
- [x] 10d.1 Test observer activation: pending `prompt_request`, principal below `control` · activate the control · no `prompt_response`, session still blocked, interaction still acknowledged (test-plan #X19; see `plugin-action-handler.test.ts`)
- [x] 10d.2 Test bystander answering: question raised by principal A's command, principal B at `control` · B activates · no `prompt_response` sent (test-plan #X20; see `plugin-action-handler.test.ts`)
- [x] 10d.3 Test question with no chat invoker: attached session raises a question · any `control` principal on the binding answers · `prompt_response` delivered, no deadlock (test-plan #X21; see `plugin-action-handler.test.ts`)

### 10e. Audit (L1)

- [x] 10e.1 Test retention at the bound: log holding limit−1 entries with limit 10,000 · append one, then one more · at limit nothing discarded; at limit+1 oldest gone, count equals limit, newest present (test-plan #E21; see `preferences-store.test.ts`)
- [x] 10e.2 Test default retention: config with no retention key · read the effective limit · 10,000 (test-plan #E22; see `config-redact.test.ts`)
- [x] 10e.3 Test one entry per action-bearing request: one permitted prompt, one refused prompt, one mirrored event · process all · exactly 2 entries, none for the mirrored event (test-plan #E23; see `plugin-action-handler.test.ts`)
- [x] 10e.4 Test refusal reasons recorded distinctly: one request per refusal class · process all · each entry carries its own distinct reason (test-plan #X23; see `plugin-action-handler.test.ts`)
- [x] 10e.5 Test provenance persists: session first driven from chat · restart the dashboard · the plugin-owned ref survives naming principal+channel+binding, user tag namespace untouched (test-plan #X24; see `plugin-enabled.test.ts`)
- [x] 10e.6 Test the log has no edit path: existing log entries · enumerate the layer's exposed interface · no edit or delete operation exists (test-plan #X25; see `plugin-action-handler.test.ts`)
- [x] 10e.7 Test the log is unreachable from chat: `operate` principal asks the bot for the log · process · refused and redirected to the dashboard with no log content posted (test-plan #X26; see `plugin-action-handler.test.ts`)

### 10f. Host workspace seam (L1)

- [x] 10f.1 Test defensive copy: `listWorkspaces()` result · mutate the array and a nested `folders` array · store unchanged and a re-read returns original state (test-plan #E26; see `preferences-store.test.ts`)
- [x] 10f.2 Test empty state: no workspaces configured · `listWorkspaces()` · empty collection with no throw (test-plan #E27; see `preferences-store.test.ts`)
- [x] 10f.3 Test every mutator notifies: subscriber registered, all 9 store mutators · invoke each in turn · 9 notifications, one per mutator (test-plan #E28; see `preferences-store.test.ts`)
- [x] 10f.4 Test the completeness guard: notification wiring removed from one mutator · run the guard · guard fails naming that mutator (test-plan #E29; see `preferences-store.test.ts`)
- [x] 10f.5 Test unsubscribe: subscriber that has unsubscribed · mutate a workspace · handler not invoked (test-plan #E30; see `plugin-enabled.test.ts`)
- [x] 10f.6 Test throwing subscriber: two subscribers where the first throws · mutate a workspace · second still notified and the mutation persists (test-plan #X22; see `plugin-enabled.test.ts`)
- [x] 10f.7 Test additivity: a plugin written before the seam existed · load it · activates unchanged with no context member shape change (test-plan #X30; see `plugin-enabled.test.ts`)
...
- [x] 10g.7 Spec elision marker rendering: assistant message exceeding the platform message limit · mirror it · thread shows an explicit elision marker (test-plan #F7; see `tests/e2e/blackhole-settings.spec.ts`) — LEVEL CHANGED to L1 (`gateway-team-controls.test.ts` "F7"): drives the real `handleFrame` → `renderMirror` → pacer → adapter chain and asserts the marker IS present, the post fits the 2000-char platform limit, and visible+reported == the original length (so the marker cannot under-report). NOT writable at L3 without new infrastructure: the mirror lane fires on SESSION frames and the harness cannot inject a synthetic frame into a live session — the fake platform fakes only the CHAT side (`inbound.jsonl`/`outbound.jsonl`), so nothing can reach the pacer inside a container. Test-plan itself constrains the change to "no new harness or test level".
...
## Discovered by the pre-merge review (ship-it step 4.5)

Blockers found by the required `@review` gate, NOT by the harness or the unit
suite — both of which were fully green. Two of the three were checkmarks that
lied (see 3.10 / 10a.16 above, now un-marked).

- [x] 11.1 **RESOLVED by operator decision: option (b) — DMs are enrollment-only.** Scope settled: pairing stays available over DM (it is also the enrollment path for guild channels, since L1 gates ALL talk on the allowlist), and DM session control is refused BY DESIGN. Implemented: the pairing reply no longer promises "you can now talk to sessions"; a DM refused `unbound_channel` is told to use a workspace-bound channel; every other refusal reason still reports itself verbatim. Spec delta gains "Requirement: Direct messages are enrollment-only". With team controls configured, EVERY direct message is refused `unbound_channel`, because a DM can never have a workspace binding. This silently regresses the prerequisite change's shipped DM pairing: the gateway still replies `"Paired. You can now talk to sessions."` BEFORE the chokepoint, then refuses every subsequent message — a dead flow that advertises itself as working. Resolution is a planning choice: (a) scope the team chokepoint to guild channels and their threads, leaving DMs under the base L1/L2 posture — consistent with `design.md`'s rollback claim that "chat-gateway's own L1/L2 posture governs again"; or (b) accept the DM removal and update the pairing UX. Note `chat-gateway-tier-authorization/spec.md:23` ("No action path SHALL reach a session without a grant") conflicts with (a) as currently written, so **the spec must change either way** — which is why this must not be settled headlessly.
- [x] 11.2 **Per-thread mirror level — FIXED.** Authorization now inherits the parent binding (fixed in 804234c21), but the mirror LANE resolves its channel from the binding-store entry (`store.get(channelKey).channelId`) and `Binding` carries no `parentChannelId` — so a session spawned INSIDE a thread mirrors at the default level instead of the binding's, violating `chat-gateway-output-filter/spec.md:30` ("an operator switches a thread from names-only to full transcript"). Needs a `Binding.parentChannelId` field, its persistence, and the mirror call site to pass it. Done: `Binding.parentChannelId` + `SpawnMeta.parentChannelId` are persisted at BOTH binding-creation sites (the attach path and the resolved-spawn path), and `mirrorFrame` passes it to `mirrorLevel`. Covered by the 11.2 test, whose second half drops the field and asserts the diff is filtered again — so the test proves the persisted parent is what resolves the operator's level rather than merely observing a passing run.

- [x] 11.3 **Fixed: the disarm latch now OUTLIVES the process.** A chat disarm permanently writes `disarm.json` (0600, atomic rename, best-effort) in the gateway state dir, and boot seeds the latch from it — falling back to `config.disarmed` only when nothing was ever persisted, so a dashboard-authored disarm still works on a fresh install. Every transition routes through one `setDisarmed` in the controller, because a direct assignment at a new call site is exactly how this was missed. Covered by the store unit tests, the controller persistence tests, and two boot-level tests (read-back and dashboard-write) that were each watched RED. The spec's "stays disarmed until an operator re-arms" is now true across restarts.

- [x] 11.4 **Fixed: an attach is confined to the BOUND workspace.** The interactive attach filtered candidates by `allowedRoots` alone, so when a workspace's folders were inert it adopted a session from anywhere the host may run — establishing the binding, the subscription and the mirror before the next message refused `scope_violation`. It now filters by the workspace's folders (`isWithinWorkspace`, the same predicate the chokepoint uses) and refuses, naming the workspace as the constraint instead of sending the operator after a missing `fixedMap`. Watched RED (the leaky version created a binding).
- [x] 11.5 **Resolved by decision: the latch stays GLOBAL, deliberately.** A per-binding latch would mean an operator's halt no longer stops a runaway principal in another workspace — the direction that fails OPEN, which is the wrong way for a safety control. The spec's own wording is "the layer" (not "the binding"), and the tension with per-binding tier isolation is real but benign: a principal mapped only in workspace A can halt things, but cannot thereby GAIN anything — the halt only ever removes authority. Recorded rather than left as an unexplained inconsistency.
- [x] 11.6 **Fixed (both correctness halves) — the synchronous rewrite is kept deliberately.** `isEntry` now requires a valid `outcome`, because the panel and the reason summary treat anything that is not `refused` as permitted, so a truncated or hand-edited file could manufacture evidence of an authorization that never happened. A failed write no longer throws into the message path (it reached the adapter's catch, so a full disk or a read-only state dir became a bot that stopped answering every message) and is reported through `onPersistFailure` → `/api/health.plugins[]` rather than swallowed. Both watched RED. The whole-log rewrite per append remains asynchronous-by-nature: batching it would trade durability for latency, which is the wrong trade for an audit trail.

**Deliberately NOT fixed here:** 11.1 changes the security posture AND requires a spec edit, and 11.2 needs a persisted-schema change. Both are planning-level, so per ship-it step 5 this hands back rather than being settled headlessly.
