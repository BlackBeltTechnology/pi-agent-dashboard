...
- [ ] 4.1 Trigger a `ci-electron.yml` dispatch on `legs: win32-x64` after the fix lands on the feature branch.
- [ ] 4.2 Download the artifact, unzip on a clean Windows VM (or `C:\test-fresh\`).
- [ ] 4.3 Launch `pi-dashboard.exe`. Verify:
...
- [ ] 5.1 Build locally on macOS (`npm run electron:make`). Launch.
- [ ] 5.2 Verify wizard still appears + functions. Confirm no double-flash of splash.
- [ ] 5.3 Confirm `await showWelcomeStep()` still resolves on wizard close.

## 6. Documentation

- [ ] 6.1 Delegate `docs/electron-bootstrap-flow.md` update to a general-purpose subagent. The state-machine diagram doesn't change (states are correct), but the doc SHOULD note the splash-close-before-wizard-open ordering as an invariant. Add ~2 lines under the `wizard-welcome` row of the States table.
- [ ] 6.2 Delegate `docs/file-index-electron.md` row update for `main.ts` noting the splash-wizard ordering invariant via "See change: fix-wizard-occluded-by-splash."
