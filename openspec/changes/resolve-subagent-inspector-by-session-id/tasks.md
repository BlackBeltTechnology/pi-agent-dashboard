...
- [ ] 7.7 L2 route running-resolve: session `subagents` dual-indexes a RUNNING run (non-empty `entries`) under both `A` and `S`; render `SubagentPopoutPage` with `agentId=S` → live timeline body renders, not-found placeholder NOT shown. See `packages/subagents-plugin/src/client/__tests__/SubagentPopoutPage.test.tsx`. (test-plan #F1)
- [ ] 7.8 L2 route backfill-resolve (PRIMARY): session `subagents` dual-indexes a COMPLETED (backfilled) run under both `A` and `S`; render `SubagentPopoutPage` with `agentId=S` → rehydrated body renders (not placeholder). See `SubagentPopoutPage.test.tsx`. (test-plan #F2)
- [ ] 7.9 L2 route unknown-id regression: render `SubagentPopoutPage` with an id present in NEITHER the `A` nor `S` slot → the ACTUAL route placeholder "Subagent not found — it may have been cleared from the parent session's history." IS shown (no false resolve). See `SubagentPopoutPage.test.tsx`. (test-plan #F3)
