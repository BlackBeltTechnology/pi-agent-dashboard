...
### Requirement: A dispatch that does not start a flow is reported as a failure

The plugin SHALL treat a flow dispatch as a **failure** when the runtime declines
to start the flow (a flow already running, an unknown flow, or a blocked gate),
and SHALL surface it to the caller; delivery of the run event SHALL NOT by itself
be reported as success. This SHALL hold for both the REST dispatch path and the
`/<flow-name>` slash-command path; the slash path SHALL settle with an error
rather than a silent acknowledgement.

#### Scenario: Dispatch into a busy session fails, not silently succeeds

- **WHEN** a flow is dispatched into a canonical session that is already running
  a flow, and the runtime declines to start it
- **THEN** the dispatch SHALL be reported as a failure to the caller
- **AND** it SHALL NOT be reported as success

#### Scenario: A started dispatch succeeds

- **WHEN** a flow is dispatched and the runtime starts it
- **THEN** the dispatch SHALL resolve successfully with the session id
