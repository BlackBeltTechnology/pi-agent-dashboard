...
- **Live footprint is observable and asserted.** A guard spec (or lifecycle probe) reads the container's own `memory.current` / `pids.current` and fails when live sessions exceed a declared budget — so reap-regressions surface as one clear failure at the point of breach, not as a mystery collapse 40 specs later.
...
- `docker-test-harness`: new requirement — the container SHALL bound its live-session footprint so a full suite run stays inside the declared memory cap, and SHALL make a dead daemon distinguishable from a failing test.
- `playwright-e2e-qa`: new requirement — a spec SHALL release the sessions it spawns before the next spec runs, with an explicit opt-out for specs whose subject is a surviving session.
