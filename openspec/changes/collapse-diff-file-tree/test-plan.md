...
| F4 | Bottom group: deleted + other | state-pure | L1 | automated | one session-owned deleted file (in `changes`, absent from on-disk set) + one `otherChanges` entry | mount tree | both appear in the muted bottom group; toggling `this session only` hides only the `otherChanges` row, deleted file stays |
