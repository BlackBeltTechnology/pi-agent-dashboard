...
- [ ] 2.1 Add a `.mutation-journal/` directory resolved from `repoRoot` (not
      cwd), so a run in a worktree journals into that worktree.
- [ ] 2.2 Split the journal write out of `applyMutation` as its own callable
      step (design D8), and write the entry BEFORE `fs.writeFileSync` mutates
      the source.
- [ ] 2.3 Write ONE entry file per mutation, temp-then-`fs.renameSync`, created
      in exclusive mode — never a single shared file rewritten per mutation
      (design D1a).
- [ ] 2.4 Store `{ path, originalBytes, mutatedBytes }` with `path`
...
- [ ] 2.5 Remove the journal entry after the existing `finally` restore succeeds.
- [ ] 2.6 Refuse to journal a mutation for a source file that already has an
      entry — exclusive create, no overwrite, no source write (design D7).
- [ ] 2.7 Add `.mutation-journal/` to `.gitignore`.

## 3. Reconciliation path

- [ ] 3.1 Export `reconcile(repoRoot)` from the harness; it scans the journal
...
- [ ] 3.4 Treat an unparseable entry, and an entry whose source file no longer
      exists, as conflicts: touch nothing, keep the entry, report it
      (design D1b, D3 row 4).
- [ ] 3.5 Include the unblock instruction in every conflict report — restore the
      file to a recorded side, or delete the named entry (design D3a).
- [ ] 3.6 Report every restored path loudly on a clean restore, and let the run
      PROCEED; fail non-zero only when an entry could not be cleanly resolved
      (design D4).
- [ ] 3.7 Proceed silently when the journal is empty or absent.
- [ ] 3.8 Wire `reconcile` into a root-level vitest `globalSetup` in
...
- [ ] 3.9 Add a `--reconcile` CLI entry to `scripts/mutation-harness.mjs` for
...
- [ ] 4.2 The handler restores, removes its entries, and TERMINATES non-zero — it
      never resumes the in-flight check against a restored file (design D4a).
- [ ] 4.3 Verify this does NOT mask the journal path — `SIGKILL` must still be
...
- [ ] 5.0 Add the shared temp-repoRoot fixture helper (fake source file +
      manifest + teardown) that the rows below build on; see
      `scripts/__tests__/repair-meta-source.test.mjs`.
- [ ] 5.1 Journal precedes the source write — input: temp repoRoot with a known
...
- [ ] 5.2 A normal restore leaves no entry — input: temp repoRoot, one mutation,
      ...
      `scripts/__tests__/async-semantics-mutation.test.mjs`.
- [ ] 5.3 One entry per mutation — input: temp repoRoot with mutation A journaled
      ...
      `scripts/__tests__/async-semantics-mutation.test.mjs`.
- [ ] 5.4 A torn entry cannot destroy a live one — input: entry A live plus a
      ...
      `scripts/__tests__/repair-meta-source.test.mjs`.
- [ ] 5.5 Restore is byte-exact — input: source file starting with a UTF-8 BOM
...
- [ ] 5.6 Entry paths are repo-relative — input: temp repoRoot with one mutation
...
- [ ] 5.7 A clean start is silent — input: temp repoRoot with an absent journal,
...
- [ ] 5.8 A second concurrent run is refused — input: an existing entry for
      ...
      `scripts/__tests__/async-semantics-mutation.test.mjs`.
- [ ] 5.9 Add the killable child-harness fixture (spawn the harness as a real
...
- [ ] 5.10 Recoverable after `SIGKILL` — input: a child harness process against a
...
- [ ] 5.11 Uncommitted work survives — input: temp git repo, file committed then
      ...
      `scripts/__tests__/repair-meta-source.test.mjs`.
- [ ] 5.12 A conflict is refused — input: residue after a kill, then the file
      ...
      `scripts/__tests__/async-semantics-mutation.test.mjs`.
- [ ] 5.13 A conflict report names its unblock — input: the 5.12 conflict state ·
      ...
      `scripts/__tests__/async-semantics-mutation.test.mjs`.
- [ ] 5.14 An unreadable entry is a conflict — input: an entry with invalid JSON,
      ...
      `scripts/__tests__/repair-meta-source.test.mjs`.
- [ ] 5.15 A journaled file that no longer exists is a conflict — input: a live
      ...
      `scripts/__tests__/repair-meta-source.test.mjs`.
- [ ] 5.16 A kill between journal and source write reconciles to a no-op — input:
...
- [ ] 5.17 A conflict blocks every project — input: a conflicting entry present at
      ...
      `scripts/__tests__/repair-meta-source.test.mjs`.
- [ ] 5.18 A clean restore does NOT block the run — input: a cleanly recoverable
      ...
      `scripts/__tests__/repair-meta-source.test.mjs`.
- [ ] 5.19 Reconcile precedes every project fork — input: a recoverable entry for
      ...
      `scripts/__tests__/async-semantics-mutation.test.mjs`.
- [ ] 5.20 `SIGINT` restores and stops — input: a child harness process
...
- [ ] 5.21 The existing `finally` still restores on a throw — input: temp
...
- [ ] 6.6 Run `review-code` before commit.
...
- [ ] 7.3 Record the single-writer assumption (one harness run per worktree at a
