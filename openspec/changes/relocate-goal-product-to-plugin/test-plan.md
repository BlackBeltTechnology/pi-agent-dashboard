...
| E26 | D2 restore path does not link (divergence, accepted) | state-transition | L1 | automated | `s1` pid-registry entry carries `pluginRef { goalId: "g1" }`; `g1.driverSessionId = "s2"`; `s1` re-registers with **no** pending token | bridge `session_register` for `s1` | `onSessionResolved` **not** dispatched to plugin `"goal"`; `g1.driverSessionId` still `"s2"`; no primer; `s1` in-memory `goalId` merged from the ref (core merge unchanged) |
...
| F2 | supervisor respawn after driver death | state-transition | L3 | automated | F1 state; kill driver via `POST /api/session/:id/abort` (or pi `shutdown`) | poll goals + sessions | converges to: new session id with same `goalId`; old session's `goalId` absent in `GET /api/sessions`; old `.meta.json` **still** has `goalId` (C2e memory-only); primer prompt count on new session === 1 |
