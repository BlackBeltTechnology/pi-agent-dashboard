...
- Each entry gains an optional `count` (default `1`) so the same action can be spawned X times in parallel.
...
- NOT breaking: a file with a single `action:` behaves exactly as today (one child, count 1).
...
- `packages/automation-plugin/src/server/` — `automation-schema.ts` (validate `actions`/`count`/bound), `engine.ts` (`startRunFor` fan-out, per-child dispatch/capture/finalize, parent aggregate, `stopRun` cascade, reaper), `runner.ts` (fire-level policy unchanged, parent keyed), `run-store.ts` (child records under the parent run dir), `routes.ts` (child data in list/result/stop).
