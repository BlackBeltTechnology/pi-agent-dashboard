...
- [ ] 1.1 Author the branch-delete outcome tests in `packages/server/src/__tests__/git-worktree-lifecycle-ops.test.ts` (see that file's existing `removeWorktree` temp-repo cases for harness glue). Triple: a worktree whose branch is merged into its base, and one whose branch is unmerged · `removeWorktree({ deleteBranch: true })` on each · merged → removed with `branchDeleted: true` and the branch gone; unmerged → still removed, `branchDeleted: false`, `branchDeleteCode: "unmerged"`, branch still exists (test-plan #E16)
- [ ] 1.2 Author the null-branch skip test in `git-worktree-lifecycle-ops.test.ts` (see the existing porcelain-parser cases for fixture shape). Triple: entries `{detached: true, branch: null}` and `{bare: true, branch: null}` · `removeWorktree({ deleteBranch: true })` · `branchDeleted: false`, `branchDeleteCode: "no_branch"`, and `git branch` never invoked — assert on the command spy, not the outcome (test-plan #E8). Do NOT author a `bare` *removal* test: `git worktree add` has no `--bare` and a porcelain `bare` entry is only ever a bare repo's own main entry, which `git worktree remove` refuses
- [ ] 1.3 Author the code-disjointness test in `packages/server/src/__tests__/git-worktree-lifecycle.test.ts` (see its existing `mapRemoveStderr` table tests). Triple: the `BranchDeleteCode` and `RemoveCode` unions · evaluate set intersection · intersection is empty, with `"git_failed"` in `RemoveCode` only (test-plan #E7)
- [ ] 1.4 Author the no-force-retry test in `packages/server/src/__tests__/git-worktree-lifecycle-routes.test.ts` (see its existing 409-mapping cases). Triple: unmerged branch removed with `deleteBranch: true` · removal succeeds, branch delete refused · HTTP 200 carrying `branchDeleted: false` + `branchDeleteCode: "unmerged"`, and NOT a `RemoveCode` of `branch_not_merged`, so a client keyed on `RemoveCode` does not auto-tick `--force` (test-plan #X4)
- [ ] 1.5 Author the single-endpoint main-worktree rejection test in `git-worktree-lifecycle-routes.test.ts` (see its existing `validateCwd` rejection cases). Triple: the main worktree's own path · `POST /api/git/worktree/remove` · failure carrying `is_main_worktree`, no `git worktree remove` runs, status is not 500 (test-plan #E14)
- [ ] 1.6 Author the abandoned-caller test in `git-worktree-lifecycle-routes.test.ts`. Triple: client disconnects after `git worktree remove` succeeds but before the response is written, with `deleteBranch: true` · abort the request mid-handler · observable blocked pending clarification C2 in test-plan.md — resolve C2 before authoring (test-plan #X6)
- [ ] 1.7 Extend `removeWorktree()` in `packages/server/src/git-worktree/git-operations.ts` with `deleteBranch?: boolean`, running `git branch -d <branch>` in the resolved main worktree after a successful removal (never `-D`, per design D5), skipping when `branch == null`. Capture the branch name **before** the removal — `removeWorktree` does not read it today and it is unrecoverable once the worktree is gone
- [ ] 1.8 Define `BranchDeleteCode = "deleted" | "unmerged" | "no_branch" | "branch_gone" | "delete_failed"` in a namespace disjoint from `RemoveCode` — the generic failure is `delete_failed`, NOT `git_failed`, which is already a `RemoveCode` value; and it must never reuse `branch_not_merged`, which means "removal failed" and makes `CloseWorktreeDialog.tsx:60` auto-tick `--force` and retry
- [ ] 1.9 Widen the success payload to `{ removed: true; branchDeleted: boolean; branchDeleteCode?: BranchDeleteCode }` and thread `deleteBranch` through `POST /api/git/worktree/remove` in `packages/server/src/routes/git-routes.ts`
- [ ] 1.10 Add the `is_main_worktree` rejection to the single `remove` endpoint — it has no `isMain` guard today, so removing the main worktree maps to `git_failed` → 500 rather than a clean rejection
- [ ] 1.11 Verify tasks 1.1–1.5 pass and every existing `remove` test still passes with `deleteBranch` absent (default behaviour unchanged)

## 2. Server — `remove-batch` endpoint

- [ ] 2.1 Author the partial-failure test in `packages/server/src/__tests__/git-worktree-lifecycle-routes.test.ts` (see its existing multi-worktree temp-repo setup). Triple: a 3-item batch where item 2 has uncommitted changes · `POST /api/git/worktree/remove-batch` without force · 3 results in input order, items 1+3 `ok` and gone from disk, item 2 `dirty_worktree` and still on disk (test-plan #X1)
- [ ] 2.2 Author the blocked-item test in the same file (see the single endpoint's `active_sessions` 409 case for the session-manager stub). Triple: item 2 has 2 active sessions under its path · batch post without force for that item · item 2 reports `active_sessions` with its own `sessionIds`, items 1+3 still process (test-plan #X2)
- [ ] 2.3 Author the per-item containment test in the same file. Triple: a batch where item 2's `cwd` fails `validateCwd` or resolves outside the main worktree · batch post · item 2 rejected with the validation code, items 1+3 still process, nothing outside the repo touched (test-plan #X10)
- [ ] 2.4 Author the cwdMissing-broadcast test in the same file, asserting against the browser-gateway spy the single-endpoint tests already use. Triple: a successfully removed batch item with 2 sessions registered under its path · batch post succeeds for that item · both sessions updated with `cwdMissing: true` and one `sessionUpdated` broadcast emitted per session (test-plan #X3)
- [ ] 2.5 Author the body-validation test in the same file. Triple: bodies `{}`, `{items: null}`, `{items: "abc"}`, `{items: {}}` · `POST /api/git/worktree/remove-batch` · each returns 400 with the stable validation code and zero git commands execute (test-plan #E11)
- [ ] 2.6 Author the cap lower-boundary test in the same file. Triple: batch arrays of length 0 and 1 · batch post · length 0 accepted with `results: []` and no git invoked; length 1 processed normally (test-plan #E5)
- [ ] 2.7 Author the cap upper-boundary test in the same file. Triple: batch arrays of length 50 and 51 · batch post · 50 processed returning 50 results; 51 returns 400 with the stable cap code and zero git commands execute (test-plan #E6)
- [ ] 2.8 Register `POST /api/git/worktree/remove-batch` with the `networkGuard` preHandler, applying `validateCwd` + main-worktree resolution per item, never aborting on first failure (design D4)
- [ ] 2.9 Widen the item result to `code: RemoveCode | "active_sessions" | "cwd_invalid" | "is_main_worktree"` plus `sessionIds?: string[]` on the `active_sessions` case — `active_sessions` is a route-level 409 and is not a `RemoveCode` member, so without this the per-row escalation is unimplementable
- [ ] 2.10 Implement the `cwdMissing` stamp + broadcast per successful item, extracting the block shared with the single endpoint (`git-routes.ts:641-648`) rather than duplicating it
- [ ] 2.11 Enforce the batch item cap (50) before any git invocation; server-side guard only — do NOT mirror the constant client-side
- [ ] 2.12 Verify tasks 2.1–2.7 pass

## 3. Server — `prune` endpoint

- [ ] 3.1 Author the prune-clears test in `packages/server/src/__tests__/git-worktree-lifecycle-ops.test.ts` (see its temp-repo worktree fixtures). Triple: one registration whose directory was deleted outside git · `POST /api/git/worktree/prune` · that registration is gone from `git worktree list` and the response reports the pruned count (test-plan #X8)
- [ ] 3.2 Author the prune-no-op test in the same file. Triple: every registration's directory exists · prune called · succeeds reporting 0 pruned, no registration removed (test-plan #X7)
- [ ] 3.3 Author the guard test in `git-worktree-lifecycle-routes.test.ts` (see its existing `networkGuard` rejection cases). Triple: a request the guard denies · call `remove-batch` and `prune` · both rejected before any git command runs; both also respond (not 404) when the guard allows (test-plan #X9)
- [ ] 3.4 Add `pruneWorktrees(cwd)` to `git-operations.ts` wrapping `git worktree prune` in the resolved main worktree, returning the pruned count
- [ ] 3.5 Register `POST /api/git/worktree/prune` with the `networkGuard` preHandler and `validateCwd`
- [ ] 3.6 Verify tasks 3.1–3.3 pass

## 4. Server — per-entry directory existence

- [ ] 4.1 Author the existence-reporting test in `packages/server/src/__tests__/git-worktree.test.ts` (see its `parsePorcelainWorktrees` cases). Triple: a repo with 3 registrations, one directory deleted outside git · `GET /api/git/worktrees` · the deleted one reports `exists: false`, the other two `exists: true`, every other field unchanged from the pre-change shape (test-plan #E15)
- [ ] 4.2 Author the statSync-cost check in the same suite, timed. Triple: `GET /api/git/worktrees` on a repo with 50 registrations · 20 runs · added wall-clock stays within the noise of the existing `git worktree list` `execSync` cost, comparing medians (test-plan #P2)
- [ ] 4.3 Add `exists: boolean` to **both** `WorktreeEntry` declarations — `packages/server/src/git-worktree/git-worktree.ts:30` and the client mirror at `packages/client/src/lib/git/git-api.ts:144` (there is no shared declaration) — and populate it where the porcelain output is parsed/served; additive, no protocol bump (design D8)
- [ ] 4.4 Verify no existing consumer of `WorktreeEntry` breaks on the widened type
...
- [ ] 9.2 Author the batch-blocking perf check in `qa/tests/` as a CLI/process smoke row with no rendered-UI assert. Triple: a 50-item `remove-batch` against 50 real temp worktrees with a bridge WS client connected and heartbeating · run one batch · threshold blocked pending clarification C1 in test-plan.md — resolve C1 before authoring; if C1 resolves to "no budget", drop this rather than weaken it to an exit-0 assertion (test-plan #P1)
