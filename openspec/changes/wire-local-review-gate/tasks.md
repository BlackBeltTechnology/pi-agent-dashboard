...
- [ ] 1.1 Re-derive on the current tree: over-cap `AGENTS.md` files, ASCII-diagram violations under the narrow detector, `qa/tests/*.sh` browser violations, proposals missing `## Discipline Skills`, and the `kb dox lint` issue-kind census. Record the measured values
- [ ] 1.2 Confirm the design's stated figures still hold (1 over-cap, 4 ASCII, 0 shell-browser, 34/74, 59 dox issues); update `design.md` D6/D8 if the tree moved

## 2. Repair the broken enforcers before wiring anything

- [ ] 2.1 Repair **both** stale paths in `scripts/i18n-parity.mjs` — `lib/i18n.tsx` AND `lib/i18n-hu.ts`, both moved under `lib/i18n/`
- [ ] 2.2 Run `node scripts/i18n-lint.mjs --strict`; clear any hits, or fix them here

## 3. Clear the mechanical violations (so the gate lands green)

- [ ] 3.1 Convert the ASCII box-drawing diagrams to ```mermaid blocks (delegate `docs/` prose writes to DocScribe, caveman style)
- [ ] 3.2 Split the over-cap `AGENTS.md` with `node scripts/split-large-agents.mjs <path> --write`; verify under 30000 bytes
...
- [ ] 5.1 Implement the four rules with the D6 detectors — no new dependency, per-rule reporting, non-zero exit on any gating violation
- [ ] 5.2 Implement touched-set resolution behind an explicit `--base <ref>` flag, filtering `--diff-filter=AM`, excluding pure renames. No `--base` → Discipline-Skills reports without gating; mode is never inferred

## 6. Implementation — the dox byte-arm gate

- [ ] 6.1 Implement the byte-arm gate as a thin consumer of `kb dox lint --json`, failing only on `over-threshold` / `arm:"bytes"` — filter only, recompute no threshold, no walk, no classification
- [ ] 6.2 Do NOT add a `--check` mode to `split-large-agents.mjs`

## 7. Wire step 4.4 into ship-it (NOT into quality:changed)

- [ ] 7.1 Add step 4.4 to `.pi/skills/ship-it/SKILL.md`, after the step-3 harness gate and before 4.5: `check-conventions.mjs --base origin/develop`, the byte-arm dox gate, `i18n:lint --strict`, `i18n:parity`
- [ ] 7.2 Specify that a 4.4 failure routes to the step-4 fix loop and that step 4.5 does not run

## 8. ship-it step 4.5 — the local review gate

- [ ] 8.1 Add step 4.5 to `.pi/skills/ship-it/SKILL.md`, strictly before the step-6 inline `ship-change` drive
- [ ] 8.2 Pin the engine contract: an `Agent` spawn with `model: "@review"` carrying `review-code`'s rubric — never an inline self-review, never the CodeRabbit CLI
- [ ] 8.3 Specify `@review` as REQUIRED: unconfigured → hard fail naming `update_roles` / the Roles panel and suggesting a `@propose-review-N` seed; no fallback to the session default
- [ ] 8.4 Give the required role an onboarding path: document `@review` in setup + a one-time interactive bootstrap on first hard-fail; non-interactive runs keep the hard fail
- [ ] 8.5 Specify the reviewer input: change diff + `proposal.md` + task text, diff scoped to the change's own work
- [ ] 8.6 Wire step 4.5 to `reviewRoundDecision`; specify the hard two-round cap and per-invocation timeout
- [ ] 8.7 Specify severity routing and the `assertNoWeakening` escalation valve
- [ ] 8.8 Specify that every halt cause writes `SHIP_IT_BLOCKED.md` and exits non-zero — no new artifact or exit code
- [ ] 8.9 Update ship-it Guardrails, Composed skills (`review-code`), and the Mermaid flowchart
- [ ] 8.10 Record explicitly that there is no triviality escape

## 9. Spec-posture change for Discipline Skills

- [ ] 9.1 Update `AGENTS.md` OpenSpec Conventions: heading required on any touched proposal; when no discipline applies, say so under the heading rather than omitting it
- [ ] 9.2 Verify this change's own `proposal.md` carries the heading

## 10. Docs

- [ ] 10.1 DocScribe: update `docs/code-quality.md` with the five oracle layers and where each is invoked (note `quality:changed` is a dev-loop oracle with no automated caller)
- [ ] 10.2 DocScribe: record the ast-grep negative result with re-derived figures (678 hex, 451 in `lib/theme/` token files); note the originals cited paths deleted by the client reorg
- [ ] 10.3 Add directory `AGENTS.md` rows for `scripts/check-conventions.mjs` and the new decision helper
...
- [ ] 13.1 `set -o pipefail; npm test 2>&1 | tee /tmp/pi-test.log` green; grep the log for the summary line
- [ ] 13.2 `node scripts/check-conventions.mjs` exits 0 on the change's own tree
- [ ] 13.3 The byte-arm dox gate, `i18n:lint --strict`, and `i18n:parity` all exit 0 on the change's own tree (raw `kb dox lint` still exits 1 on the 58 out-of-scope issues — expected)
