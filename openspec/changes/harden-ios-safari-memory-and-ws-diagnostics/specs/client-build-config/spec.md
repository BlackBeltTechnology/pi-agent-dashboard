...
- The icon-by-key resolver SHALL keep resolving arbitrary extension-supplied keys (the full namespace stays available once loaded). Until the set has loaded, a key-resolved icon SHALL render nothing, then render the icon once the set is available, without throwing and without a layout-breaking placeholder.
...
- **THEN** no chunk referenced by `index.html` (entry script or `modulepreload`), nor any chunk statically imported by them, contains the `@mdi/js` export marker `mdiZodiacAquarius`
