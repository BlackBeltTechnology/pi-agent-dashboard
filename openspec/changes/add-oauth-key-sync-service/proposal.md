...
**The service is a provider-shaped proxy, not a credential distributor.** Members point pi at the keysync endpoint and authenticate with a keysync-issued key; keysync holds the pooled provider accounts and forwards upstream on their behalf. No provider OAuth token ever reaches a member's machine.
...
- **Several accounts per provider, captured client-side and uploaded.** A member adds an account by running pi's own login against a scratch config directory via `PI_CODING_AGENT_DIR` (`docs/environment-variables.md`), harvesting the resulting credential, uploading it to keysync, and discarding the scratch directory. Their existing credential is never overwritten and no provider OAuth flow is reimplemented.
- **Per-member primary selection.** Each member marks one account per provider as their primary; it is selected whenever healthy, and rotation returns to it once it recovers.
...
- `keysync-authz`: Roles (`admin` / `member` / `revoked`), super-admin grant and revoke, and enforcement on every proxy and management route. Revocation takes effect on the member's next request.
...
- `keysync-enrolment`: Capture an additional provider account by running pi's login against a scratch `PI_CODING_AGENT_DIR`, then upload it to the pool without disturbing the member's existing credential.
...
- `keysync-primary-selection`: Per-member, per-provider primary account; selected whenever healthy and returned to after a rotation once it recovers.
...
- Enrolment shells pi's own login against a scratch `PI_CODING_AGENT_DIR`. If that variable is withdrawn upstream, enrolment needs another capture mechanism; forwarding and rotation are unaffected.
