...
The timeout is a LAST RESORT for a prompt whose fate is genuinely unknown. It
SHALL NOT be the mechanism by which a *known* delivery failure surfaces. When
`send` returns a verdict showing the prompt was never transmitted, the prompt
SHALL be marked failed immediately, and the timeout SHALL NOT be armed for it.

The existing timeout message attributes the failure to the session ("the prompt
may not have been received"). That wording SHALL be reserved for the genuinely
unknown case. A prompt known not to have left the browser SHALL surface a message
attributing the failure to the dashboard connection.
...
#### Scenario: A known-undelivered prompt fails without waiting
