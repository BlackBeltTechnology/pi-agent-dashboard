# roles-rest-api Specification

## Purpose
Session-less, read-only REST surface for the global role → model slice of
`~/.pi/agent/providers.json`. Lets a browser client with no active pi session
read which model each role uses (defaults overlaid, `builtinRoleNames` included)
without the in-session `roles:*` events. Read-only: no mutation route; the file
is never created or written.
## Requirements
