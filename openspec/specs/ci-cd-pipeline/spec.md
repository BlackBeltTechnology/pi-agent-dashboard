...
#### Scenario: Variant-harness spec runs on its own leg
- **WHEN** a spec is gated on a harness environment that cannot be the shared default
- **THEN** a dedicated job SHALL boot the harness with that environment and run the spec
- **AND** the spec SHALL NOT be left skipped in every shard
