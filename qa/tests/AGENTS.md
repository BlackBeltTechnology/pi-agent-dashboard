...
| `34-session-heap-ceiling.sh` | L2 (#X3, #X6, #X7, #X8, #X10, #X12). Proves the invocation the KERNEL executes, not the one the server builds: a spawned session reports `heapSizeLimit` in [512, 812] MB and not the server's (#X6); no heap flag in the session's `NODE_OPTIONS` (`/proc/<pid>/environ` or `ps eww`), so agent tooling stays uncapped (#X7); pi's argv carries the flag and no `keeper.cjs` process does (#X8); lowering the ceiling leaves a running session at its original value (#X10); a server booted with `env -u NODE_OPTIONS -u PI_DASHBOARD_HEAP_FLAG` still reports the configured ceiling (#X12); a `pi` tool override pointing at a POSIX shell wrapper forces the no-runtime-slot fault and asserts BOTH the `[heap]` server-log line and `/api/health` `sessionHeapFallback.used` (#X3). Restores config + `tool-overrides.json` on exit. See change: bound-session-heap-and-gc-telemetry. |
| `35-access-grant-dialog.sh` | L2 (change: add-access-grant-dialog, tasks 10.55, 10.56, 10.58, 10.59; test-plan #E18, #E19, #X11, #X12). Hermetic: temp `$HOME`, `QA_PORT` (default 18321), never :8000; launcher `PI_DASHBOARD_CMD` (default `pi-dashboard`; checkout: `node packages/server/bin/pi-dashboard.mjs`). X12: `PI_DASHBOARD_DISABLE_GRANT_PROMPT=1` + enforce/promptEnabled -> capability socket gets no `grant_request`, request 403 at once, pending cwd entry `suppressedBy: disabled`. X11: `PI_DASHBOARD_GRANT_YOLO=<root>`, env session active, capability-less request under root stays 403, zero `answeredBy: yolo`. E18: `~/.SSH` vs forbidden `~/.ssh`: case-insensitive volume -> no pending cwd entry, zero YOLO roots; case-sensitive -> distinct ordinary dir; positive control (ordinary dir recorded + offered). E19 (macOS `hdiutil` Case-sensitive APFS image, else SKIP): real `canonical-subject.ts` via jiti -> `Proj`/`proj` distinct, not within, host volume folds. Verified locally against the worktree CLI. |
...
| `33-mcp-session-token.sh` | L2 (change: wire-mcp-session-token, test-plan #P1, #X8, #X9). P1: 20 sequential header-command round trips → median+p95 ms recorded (no pass/fail threshold). X8: unset-credential command fails closed; dead endpoint bounded single attempt; live server clean 401, loopback not locked out. X9: Linux-only — argv of a live header-command child carries no token (`ps` or `/proc/cmdline`, fail-closed when neither readable), `/proc/<pid>/environ` owner-only. `PI_QA_BASE` overrides the dashboard base URL (docker exec). See change: wire-mcp-session-token. |
