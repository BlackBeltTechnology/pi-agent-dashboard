...
const { fauxAssistantMessage, fauxText, fauxThinking, fauxToolCall } = piAi;
/**
 ...
 */
export function extractDashboardFragment(systemPrompt) {
...
export function flowAgentName(systemPrompt) {
...
const SLOW_TEXT = Array.from({ length: 40 }, (_unused, index) => `slow-chunk-${index}`).join(" ");
/**
 ...
 */
export function summarizeListModelsResult(context) {
...
function toolScenario(name, args) {
    return {
        script: [
...
function askScenario(method, extra) {
    return {
        script: [
            fauxAssistantMessage([fauxToolCall("ask_user", { method, ...extra })], { stopReason: "toolUse" }),
        ],
        expect: { method },
    };
}
 ...
 */
function buildLongTranscript(turns = 120) {
    const steps = [];
 ...
 */
function buildScrollTopHeavy(trailingTurns = 40) {
    ...
    const steps = [
        ...
        fauxAssistantMessage([
            fauxToolCall("bash", {
                command: `mkdir -p "$HOME/.agent-browser/tmp" && printf %s '${TINY_PNG_B64}' | base64 -d > "$HOME/.agent-browser/tmp/scroll-top-shot.png" && ` +
                    `echo "Screenshot saved: $HOME/.agent-browser/tmp/scroll-top-shot.png"`,
            }),
        ], { stopReason: "toolUse" }),
    ];
    for (let i = 0; i < trailingTurns; i++) {
        steps.push(fauxAssistantMessage([fauxText(`trailing turn ${i}`), fauxToolCall("bash", { command: `echo trail-${i}` })], { stopReason: "toolUse" }));
    }
...
export const SCENARIOS = {
    ...
    "echo-system-context": {
        script: [
            (context) => fauxAssistantMessage([fauxText(extractDashboardFragment(context.systemPrompt))]),
        ],
    ...
    "copy-surfaces": {
        script: [
            fauxAssistantMessage([
                fauxText("Here is a table and some code.\n\n| Name | Age |\n| --- | --- |\n| Alice | 30 |\n| Bob | 25 |\n\n```js\nconst x = 1;\n```"),
            ]),
        ],
        expect: { text: "Alice" },
    },
    ...
    "ask-select-roundtrip": {
        script: [
            ...
            (context) => fauxAssistantMessage([fauxText(`you picked ${lastToolResultText(context)}`)]),
        ],
        ...
        script: [
            fauxAssistantMessage([
                fauxToolCall("bash", {
                    ...
                    command: `mkdir -p "$HOME/.agent-browser/tmp" && printf %s '${TINY_PNG_B64}' | base64 -d > "$HOME/.agent-browser/tmp/e2e-shot.png" && ` +
    ...
    "tool-codex-partial-patch": {
        script: [
            fauxAssistantMessage([fauxToolCall("apply_patch", { path: "codex-partial.md" })], { stopReason: "toolUse" }),
    ...
    "tool-grok-failed-shell": {
        script: [
            fauxAssistantMessage([fauxToolCall("Shell", {
    ...
    "tool-list-models": {
        script: [
            ...
            (context) => fauxAssistantMessage([fauxText(summarizeListModelsResult(context))]),
        ],
    ...
    "canvas-write-html-beacon": {
        script: [
            fauxAssistantMessage([
                fauxToolCall("write", {
                    path: "beacon.html",
                    content: '<!doctype html><html><head><title>b</title></head><body>' +
    ...
    "canvas-declare-server": {
        script: [
            fauxAssistantMessage([fauxToolCall("canvas", { target: { kind: "server", port: 5173 }, title: "dev server" })], { stopReason: "toolUse" }),
    ...
    "canvas-declare-server-dead": {
        script: [
            fauxAssistantMessage([fauxToolCall("canvas", { target: { kind: "server", port: 59321 }, title: "dead server" })], { stopReason: "toolUse" }),
    ...
    "canvas-declare-url": {
        script: [
            fauxAssistantMessage([fauxToolCall("canvas", { target: { kind: "url", url: "https://youtu.be/dQw4w9WgXcQ" } })], { stopReason: "toolUse" }),
...
//# sourceMappingURL=faux-scenarios.js.map