...
| `faux-scenarios.ts` | Shared scenario catalog `SCENARIOS: Record<id,{script,expect}>`. → see `faux-scenarios.ts.AGENTS.md` Adds the `om-entry` scenario (`e2e_custom_entry` → `om.observations.recorded`) for E2E F15. See change: add-custom-entry-renderer-slot. |
